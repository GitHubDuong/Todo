import { Directive, ElementRef, Injector, Renderer2, ViewChild, } from '@angular/core';
...
export abstract class BaseAccountComponent {
  ...
  getAllByDisplayInsert() {
    this.chartOfAccountService.getAllByDisplayInsert().subscribe((res: any) => this.chartOfAccounts = res);
  }
  ...
  getChartOfAccountForGoods() {
    ...
    .subscribe((res: any) => {
      ...
      this.chartOfAccounts.forEach(item => item.displaySelected = item.code + ' - ' + item.name);
      this.defaultChartOfAccount = this.chartOfAccounts.find((x) => x.code == '1561');
      this.form.patchValue({ debitCode: this.defaultChartOfAccount });
    });
  }
  ...
  filterDebitCode(event) {
    ...
    if (this.fc['debitCode'].value instanceof String) {
      ...
      this.debitCodeFilter.forEach((item) => {
        item.displaySelected = item.code + ' - ' + item.name;
      });
      return;
    }
    this.debitCodeFilter = _.filter(_.cloneDeep(this.chartOfAccounts), (item) => {
      return (
        item.name &&
        item.name != '' &&
  ...
  onSelectDebitCode($event) {
    if (
      ...
      $event.displaySelected = $event.code + ' - ' + $event.name;
      this.f.patchValue({
        debitCode: $event,
  ...
  filterCreditCode(event) {
    ...
    this.creditCodeFilter.forEach((item) => {
      item.displaySelected = item.code + ' - ' + item.name;
    });
  }
  ...
  private getCreditDetailCodeSecond(callBack) {
    ...
    .subscribe((res) => {
      ...
      this.creditDetailCodeSecondFilter.forEach((item) => {
        item.displaySelected = item.code + ' - ' + item.name;
      });
  ...
  private getCreditDetailCodeFirst(callBack) {
    ...
    .subscribe((res) => {
      ...
      this.creditDetailCodeFirstFilter.forEach((item) => {
        item.displaySelected = item.code + ' - ' + item.name;
      });
  ...
  private getDebitDetailCodeFirst(callBack) {
    ...
    .subscribe((res) => {
      ...
      this.debitDetailCodeFirstFilter.forEach((item) => {
        item.displaySelected = item.code + ' - ' + item.name;
      });
  ...
  private getDebitDetailCodeSecond(callBack) {
    ...
    .subscribe((res) => {
      ...
      this.debitDetailCodeSecondFilter.forEach((item) => {
        item.displaySelected = item.code + ' - ' + item.name;
      });
