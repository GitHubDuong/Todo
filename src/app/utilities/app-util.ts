...
const AppUtil = {
  ...
  getMenus(menuCode?) {
    ...
    return menuCode ? menus.find(x => x.menuCode === menuCode) : menus;
  },
  ...
  initTheme(myVariables: any, isCustom): void {
    if (isCustom == true) {
      Object.entries(myVariables).forEach((v: any) =>
  ...
  getIntroduceTypes(): any[] {
    return [
      ...
      {
        value: IntroduceType.Payment,
        name: 'Phương thức thanh toán',
      },
      {
        value: IntroduceType.MoneyBack,
        name: 'Đảm bảo lại tiền',
      },
      {
        value: IntroduceType.ProductBack,
        name: 'Trả lại sản phẩm',
      },
      {
        value: IntroduceType.Terms,
  ...
  setShowReportReceiptHtml(content: string = '') {
    const _idFrameReceipt = document.getElementById('iframe-html-report-balance-account');
  ...
  convertStringToDate(date: string): Date {
    ...
    if (dateSplit?.length >= 3) {
      try {
        ...
        return startOfDay(
          new Date(dateNow.getFullYear(), dateNow.getMonth(), dateNow.getDay()),
        );
      }
    } else {
      return startOfDay(
  ...
  convertObjectToFormData(obj: Object) {
  ...
  sumArray(array: any[], key: string): number {
