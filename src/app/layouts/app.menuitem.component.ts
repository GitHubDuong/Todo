  ...
  template: `
    <ng-container>
      <a
        ...
        [ngClass]="item.class"
        *ngIf="item && (!item.routerLink || item.items) && item.visible !== false"
      ...
      <ul
        *ngIf="item && item.items && active && item.visible !== false"
  ...
  itemClick(event: Event) {
    ...
    if (this.item.items) {
      ...
      if (this.app.isMobile()) {
        this.configService.isMenuMobile.next(false);
      }
    }
  }
