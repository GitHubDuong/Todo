...
export class SalaryAdvanceService {
  ...
  public getPagingSalaryAdvance(params: any): Observable<TypeData<SalaryAdvanceModel>> {
  ...
  public getProcedureNumber(): Observable<string> {
    return this.httpClient.get(`${_prefix}/get-procedure-number`, { responseType: 'text' }).pipe(
      map((model: string) => {
        return model;
      }),
    );
  ...
  public updateSalaryAdvance(model: SalaryAdvanceModel, id: number): Observable<SalaryAdvanceModel> {
  ...
  public updateSalaryAdvanceAccept(model: SalaryAdvanceModel, id: number): Observable<SalaryAdvanceModel> {
  ...
  public submitSalaryAdvance(model: SalaryAdvanceModel): Observable<SalaryAdvanceModel> {
  ...
  public deleteSalaryAdvance(id: number): Observable<SalaryAdvanceModel | null> {
    const url: string = `${_prefix}/${id}`;
  ...
  public getSalaryAdvanceRequest(params: any): Observable<any> {
    return this.httpClient.get(`${_prefix}/salary-advance-for-user`, { params }).pipe(
  ...
  public deleteSalaryAdvanceRequest(id): Observable<TypeData<any>> {
    return this.httpClient.delete(`${_prefix}//salary-advance-for-user/${id}`, {}).pipe(
