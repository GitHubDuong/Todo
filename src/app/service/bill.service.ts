...
let _prefixOrderProduce = `${AppConstant.DEFAULT_URLS.API}/OrderProduceProducts`;
...
export class BillService {
  ...
  public createOrderProduce(Bill: Bill): Observable<Bill | null> {
    const url: string = `${_prefixOrderProduce}`;
  ...
  getBillIdNewestByType(billType: string): Observable<any> {
    const url: string = `${_prefix}/GetBillIdNewest/${billType}`;
    return this.httpClient.get(url).pipe(
      map((bill: TypeData<any>) => {
        return bill.data;
      }),
    );
  }
  ...
  public deleteNotification(id: number): Observable<NotificationResult | null> {
  ...
  readMessage(id: number) {
    ...
    return this.httpClient.get(url).pipe(catchError(BillService.handleError));
  }
  ...
  changeBillNumberByType(billId: number, billPrefix: string): Observable<any> {
