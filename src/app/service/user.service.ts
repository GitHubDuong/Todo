...
export class UserService {
  ...
  public getAllUserActive1(): Observable<TypeData<User>> {
    return this.httpClient.get(`${_prefix}/getAllUserActive1`).pipe(
      map((user: TypeData<User>) => {
        return user;
      }),
    );
  ...
  public createUser(user: User, id: any): Observable<User | null> {
    const url: string = `${_prefix}`;
    const options = {
      params: new HttpParams({ fromObject: { id: id } }),
    };
    return this.httpClient.post(url, user, options).pipe(
      map((user: User) => {
        return user;
      }),
    );
  ...
  uploadFaceImage(formData): Observable<any> {
    return this.httpClient
      .post(`${_prefix}/upload-face-image`, formData)
      .pipe(catchError(this.errorMgmt));
  }
  ...
  getUserNotRoles() {
    return this.httpClient.get(`${_prefix}/user-not-roles`);
  }

  resetPassword(body: any) {
    return this.httpClient.post(`${_prefix}/resetPassword`, body);
  }
}
