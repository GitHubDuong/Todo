...
export class CustomerContactHistoryService {
  ...
  public getCustomerContactHistoryByCustomer(
    ...
    return this.httpClient.get(`${_prefix}/get-by-customer`, { params }).pipe(
      map((customer: TypeData<CustomerContactHistory>) => {
        return customer;
      }),
    );
  ...
  getCustomerContactNotification(): Observable<any> {
    return this.httpClient.get(`${_prefix}/customer-contact-notification`).pipe(
      map((res: any) => {
        return res;
      }),
    );
  ...
  public getCountCustomerContact(): Observable<number> {
    return this.httpClient.get(`${_prefix}/count-customer-contact`).pipe(
      map((count: number) => {
        return count;
      }),
    );
  ...
  public updateCustomerHistory(user: User, id: number): Observable<User> {
    console.log(user);
    const url: string = `${_prefix}/${id}`;
  ...
  public deleteCustomerHistory(id: number): Observable<User | null> {
    const url: string = `${_prefix}/${id}`;
