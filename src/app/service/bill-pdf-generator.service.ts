...
export class BillPdfGeneratorService {
  ...
  generatePdfXK(isNonPrice: boolean) {
    // Init component
    const viewContainerRef = this.viewContainerRef;
    viewContainerRef.clear();
    ...
    // Prepare pdf data
    componentRef.instance.dataPrint = this.pdfBillData;
    componentRef.instance.isNonPrice = isNonPrice;
    setTimeout(() => {
      const body = componentRef.location.nativeElement.innerHTML;
      let htmlTemplate = `
                <!DOCTYPE html>
  ...
  calculateDiscountAmount(product: any) {
    ...
    if (product.discountType === 'percent') {
      ...
      discountAmount =
        (product.billQuantity * product.unitPrice * product.discountPrice) /
        100;
    } else {
  ...
  get discountAmount() {
    if (this.bill === undefined || this.bill === null) return 0;
