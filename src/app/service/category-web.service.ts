...
let _prefix = `${AppConstant.DEFAULT_URLS.API}/CategoryWebs`;
...
export class CategoryWebService {
  ...
  getCategory(): Observable<TypeData<any>> {
    return this.httpClient.get(`${_prefix}/list-category`).pipe(
      map((res) => {
  ...
  getPagingPromotion(params: any): Observable<TypeData<any>> {
  ...
  getGoodInventory(params: any): Observable<any> {
    return this.httpClient
      .get(`${_prefix}/get-good-inventory`, { params })
      .pipe(
        map((goods) => {
  ...
  createPromotion(request: any): Observable<any> {
  ...
  updatePromotion(body, id): Observable<any> {
  ...
  public getProcedureNumber(): Observable<any> {
    const url: string = `${_prefix}/get-procedure-number`;
    return this.httpClient.get(url, {}).pipe(
      map((res) => {
        return res;
      }),
  ...
  exportExcel(id: number): Observable<any> {
    return this.httpClient
      .get(`${_prefix}/export-inventory`, { params: { id } })
      .pipe(
        map((res) => {
          return res;
        }),
