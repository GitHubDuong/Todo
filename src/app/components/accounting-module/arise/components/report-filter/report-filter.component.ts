...
export class ReportFilterComponent implements OnInit {
  ...
  getListTaxCode() {
    const request = {
      ...
      fromDate: this.filter.filterType === 2 ? this.filter.fromDate : null,
    ...
    this.accountingReportService.getListTaxCode(request).subscribe((res) => {
  ...
  onChangeAccount(event, type) {
    if (type === 'account') {
      ...
      this.chartOfAccountService
        .getDetailHasParam(`${this.filter.accountCode}:${event.value}`, {
          pageSize: 9999,
        })
        .subscribe((res: any) => {
