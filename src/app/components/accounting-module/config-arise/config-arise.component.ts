...
export class ConfigAriseComponent implements OnInit, OnDestroy {
  ...
  listButton = [
    {
      id: ConfigButtonAriseEnum[ConfigButtonAriseEnum.nokeepDataChartOfAccount],
  ...
  onSelectFocus(functionName: string) {
    const body = {
      key: '',
  ...
  onSelectCheckBox(data: IFormConfigAriseDto) {
    ...
    const listChildrenKeys = _.filter(this.listConfigDocument, (item: any) => {
      ...
      .subscribe((res) => {
        ...
        if (this.listConfigDocument.length) {
          let item = _.find(res, (item) => {
            return item.focusFunctions.includes(this.filterParam.button);
          });
  ...
  transateConfigLabel(name: string) {
    return AppUtil.translateWithParams(
      this.translateService, 
      'label.config_auto_focus_dynamic', 
      { name });
  }
