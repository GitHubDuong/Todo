...
export class AriseCrudV2Component implements OnInit, OnDestroy {
  ...
  @Output('onResetTableViewDetail') onResetTableViewDetail = new EventEmitter();
    ...
    let callBack = () => {
      ...
      const payload = {
        ...
        orginalBookDate: new Date(
          AppUtil.convertStringToDate(this.fc['orginalBookDate'].value),
        )?.toISOString(),
      ...
      return this.ledgerService.getTotalAmountTax(payload).subscribe((res) => {
        ...
        const amount =
          (Number(res || 0) * Number(invoiceCode?.percent || 0)) / 100;
        this.f.patchValue({
          orginalDescription: {
  ...
  onF8() {
    ...
    let callBack = () => {
      ...
      // this.focusInput(this.orginalVoucherNumberTmp);
      this.autoFocusAfterSave(ConfigButtonAriseEnum.nokeepDataTax);
      this.onReloadTableAfterCrud.emit(ConfigButtonAriseEnum.nokeepDataTax);
    };
    this.saveData(_.cloneDeep(this.f.value), callBack);
  }
  ...
  onF2() {
    ...
    let callBack = () => {
      ...
      this.f.patchValue(data);
      this.autoFocusAfterSave(ConfigButtonAriseEnum.nokeepDataChartOfAccount);
  ...
  onF4() {
    ...
    let callBack = () => {
      ...
      this.onReloadTableAfterCrud.emit(ConfigButtonAriseEnum.nokeepDataBill);
    };
    this.saveData(_.cloneDeep(this.f.value), callBack);
  }
  ...
  private saveData(data: any, callBack): Subscription {
    ...
    const input = Object.assign(
      {},
      {
        ...
        orginalCompanyName:
          data['orginalCompanyName']?.name || data['orginalCompanyName'] || '',
        orginalDescription:
          data['orginalDescription']?.name || data['orginalDescription'] || '',
        invoiceCode: data['invoiceCode']?.code || data['invoiceTaxCode'] || '',
        invoiceTaxCode:
          data['invoiceTaxCode']?.taxCode || data['invoiceTaxCode'] || '',
        ...
        orginalBookDate: AppUtil.formatLocalTimezone(
          moment(
            data['orginalBookDate'] ? data['orginalBookDate'] : new Date(),
            AppConstant.FORMAT_DATE.VN_DATE_PIPE_SHORT_DATE,
          ).format(AppConstant.FORMAT_DATE.T_DATE),
        ),
        referenceBookDate: AppUtil.formatLocalTimezone(
          moment(
            data['referenceBookDate'] ? data['referenceBookDate'] : new Date(),
            AppConstant.FORMAT_DATE.VN_DATE_PIPE_SHORT_DATE,
          ).format(AppConstant.FORMAT_DATE.T_DATE),
        ),
    ...
    return this.ledgerService.createLedgerV2(input).subscribe((res) => {
      this.messageService.add({
        severity: 'success',
        detail: input.id ? 'Cập nhật thành công' : 'Thêm mới thành công',
      });
      callBack();
    });
  }
  ...
  filterInvoiceTaxCode(event) {
    ...
    var callBack = () => {
      setTimeout(() => {
        ...
        if (autocompletePanel && this.invoiceTaxCodeFilter?.length > 0) {
          this.renderer.listen(autocompletePanel, 'scroll', (event) => {
            if (
              event.target.scrollHeight - event.target.clientHeight - 10 <=
  ...
  private getInvoiceTaxCode(callBack) {
    if (
      ...
      this.invoiceTaxCodePage.totalItems === this.invoiceTaxCodeFilter?.length
    ) {
      return;
    }
    ...
    this.payerService.payerCustomerTaxs(params).subscribe((res: any) => {
      ...
      this.invoiceTaxCodeFilter = this.invoiceTaxCodeFilter.concat(res.data);
    ...
    if ($event.credit) {
      ...
      if ($event.creditDetailFirst && $event.credit.hasDetails) {
        ...
        if ($event.creditDetailSecond && $event.creditDetailFirst.hasDetails) {
          ...
          this.creditDetailCodeSecondFilter = [$event.creditDetailSecond];
        }
      }
    }
    ...
    if ($event.debit) {
      ...
      if ($event.debitDetailFirst && $event.debit.hasDetails) {
        ...
        if ($event.debitDetailSecond && $event.debitDetailFirst.hasDetails) {
          ...
          this.debitDetailCodeSecondFilter = [$event.debitDetailSecond];
        }
      }
    }
    this.setFlagCaculate();
  ...
  filterDebitDetailCodeFirst(event) {
    ...
    var callBack = () => {
      setTimeout(() => {
        ...
        if (autocompletePanel && this.debitDetailCodeFirstFilter?.length > 0) {
          this.renderer.listen(autocompletePanel, 'scroll', (event) => {
            if (
              event.target.scrollHeight - event.target.clientHeight - 10 <=
              event.target.scrollTop
  ...
  filterDebitDetailCodeSecond(event) {
    ...
    var callBack = () => {
      setTimeout(() => {
        ...
        if (autocompletePanel && this.debitDetailCodeSecondFilter?.length > 0) {
          this.renderer.listen(autocompletePanel, 'scroll', (event) => {
            if (
              event.target.scrollHeight - event.target.clientHeight - 10 <=
              event.target.scrollTop
  ...
  filterCreditCode(event) {
    ...
    if (this.fc['creditCode'].value instanceof String) {
      ...
      this.creditCodeFilter = [(event.query = this.fc['creditCode'].value)];
      return;
    }
  ...
  filterCreditDetailCodeFirst(event) {
    ...
    var callBack = () => {
      setTimeout(() => {
        ...
        if (autocompletePanel && this.creditDetailCodeFirstFilter?.length > 0) {
          this.renderer.listen(autocompletePanel, 'scroll', (event) => {
            if (
              event.target.scrollHeight - event.target.clientHeight - 10 <=
              event.target.scrollTop
  ...
  onChangeInputAutoCompleteInvoiceCode(allowAutoFocus: boolean) {
    ...
    if (invoiceCode !== '') {
      ...
      if (invoice) {
        ...
        this.setTabIndexRelatedInputInvoiceCode();
        if (allowAutoFocus) {
          this.invoiceSerialTmp.nativeElement?.setAttribute('tabIndex', 0);
          this.focusInput(this.invoiceCodeTmp);
        }
      }
    }
  ...
  private setTabIndexRelatedInputInvoiceCode() {
    try {
      _.each(
        ...
        (key) => {
          if (this[`${ConfigAriseEnum[key]}Tmp`] instanceof InputMask) {
            (
            ...
            this[`${ConfigAriseEnum[key]}Tmp`] instanceof InputNumber
          ) {
            (
          ...
          } else if (this[`${ConfigAriseEnum[key]}Tmp`] instanceof ElementRef) {
            (
            ...
            this[`${ConfigAriseEnum[key]}Tmp`] instanceof AutoComplete
          ) {
            if (this.isInvoiceCodeHasValue) {
              (
                this[`${ConfigAriseEnum[key]}Tmp`] as AutoComplete
  ...
  get isValidationInvoiceCode() {
    return this.isInvoiceCodeHasValue || this.fc['invoiceCode'].value === '';
  }
  ...
  getArisingForOriginVoucherNumber() {
    ...
    const orginalVoucherNumber: string = this.fc['orginalVoucherNumber'].value;
  ...
  buildNewForm($event: any) {
    ...
    if (!data) {
      ...
      this.f.patchValue({
        ...
        debitDetailCodeFirst: dataTable.document?.debitDetailFirst || '',
        debitDetailCodeSecond: dataTable.document?.debitDetailSecond || '',
        creditCode: dataTable.document?.credit || '',
        creditDetailCodeFirst: dataTable.document?.creditDetailFirst || '',
        creditDetailCodeSecond: dataTable.document?.creditDetailSecond || '',
        orginalVoucherNumber: orginalVoucherNumber,
        voucherNumber: voucherNumber,
    ...
    this.f.patchValue({
      ...
      invoiceAdditionalDeclarationCode: data.invoiceAdditionalDeclarationCode,
    ...
    _.each(this.listConfig, (config) => {
      if (
        ...
        if (
          ...
          result[config.configAriseBehaviour.codeData] = moment().format(
            AppConstant.FORMAT_DATE.VN_DATE_PIPE_SHORT_DATE,
          );
        } else {
  ...
  private autoFocusAfterSave(functionName: ConfigButtonAriseEnum) {
    console.log(`Focus for function ${ConfigButtonAriseEnum[functionName]}`);

    const config = _.find(this.listConfig, (item) => {
      return item.focusFunctions.includes(ConfigButtonAriseEnum[functionName]);
    });
    ...
    if (config) {
      if (
        ...
        this.focusInput(
          this[`${config.configAriseBehaviour.codeData}Tmp`] as AutoComplete,
        );
      } else if (
        this[`${config.configAriseBehaviour.codeData}Tmp`] &&
        !(
        ...
        this.focusInput(
          this[`${config.configAriseBehaviour.codeData}Tmp`] as InputMask,
        );
      } else if (
        this[`${config.configAriseBehaviour.codeData}Tmp`] &&
        !(
        ...
        this.focusInput(
          this[`${config.configAriseBehaviour.codeData}Tmp`] as InputNumber,
        );
      } else {
        this.focusInput(this.orginalVoucherNumberTmp);
      }
