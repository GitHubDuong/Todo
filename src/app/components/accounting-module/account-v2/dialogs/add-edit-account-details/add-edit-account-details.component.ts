  ...
  implements OnInit
{
  ...
  ngOnInit() {
    ...
    if (
      this.noneTabIndexRoutes.includes(this.route.snapshot.routeConfig.path)
    ) {
      this.tabIndex = -1;
    }
  }
      ...
      switch (this.currentParentAccount.accGroup) {
        case AccountGroupType.Inventory:
          return this._fb.group({
            ...
            duration: this._fb.control(this.currentParentAccount.duration, [
              Validators.required,
            ]),
            accGroup: this._fb.control(this.currentParentAccount.accGroup, [
              Validators.required,
            ]),
  ...
  show(
    ...
    if (parentData.duration == '') parentData.duration = AccountDurationType.n;
  ...
  getWareHouseList() {
    if (
      this.currentParentAccount.accGroup === this.accountGroupTypes.Inventory
    ) {
  ...
  getTitle(): Observable<string> {
    if (this.currentParentAccount.accGroup == AccountGroupType.Normal) {
      return AppUtil.translate$(
        ...
        !this.currentDetail ? 'label.add_details' : 'label.edit_details',
      );
    } else {
          ...
          this.formGroup.patchValue({
            ...
            warehouseCode:
              this.currentParentAccount.accGroup ===
                this.accountGroupTypes.Inventory && this.wareHouses?.length > 0
                ? this.wareHouses[0].code
                : '',
            duration: this.currentParentAccount.duration,
            accGroup: this.currentParentAccount.accGroup,
            classification: this.currentParentAccount.classification,
