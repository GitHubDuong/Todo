...
import { AccountGroupDetailForChildParams, ImportExportAcccountQueryParam } from 'src/app/models/account-group.model';
...
import { IIsTableColumn, IsTableColumn, IsTableColumnType } from 'src/app/shared/is-table/is-table.model';
...
export class AccountV2Component extends AppComponentBase implements OnInit, AfterViewInit {
  ...
  get codeExcelDetail() {
    return this.currentAccountDetail1s?.code;
  }
  ...
  @Input('displayHoachToanNoiBoButton') displayHoachToanNoiBoButton: boolean = true;
  ...
  onRemoveAccount(data) {
    ...
    this.confirmationService.confirm({
      ...
      accept: () => {
        this.accountGroupService.deleteAccount(data.id).subscribe((_) => {
          this.getAccounts();
  ...
  getCodeExcel(event: any = null) {
    ...
    this.accountGroupService.getListDetailForChildNode(this.codeExcel, param).subscribe((response) => {
      ...
      const request =
        arraylist?.reduce((arr: any[], curr, index) => {
          ...
          if (curr?.['type'] == 'Nội bộ') {
            ...
            item.openingForeignDebitNB = curr?.['foreignDebit'] || 0;
            item.openingForeignCreditNB = curr?.['foreignCredit'] || 0;
          } else {
            item.openingDebit = curr?.['openingDebit'] || 0;
            item.openingCredit = curr?.['openingCredit'] || 0;
            item.openingForeignDebit = curr?.['foreignDebit'] || 0;
            item.openingForeignCredit = curr?.['foreignCredit'] || 0;
          }
          let accGroup = this.accGroups.find((e) => e.value == curr?.['accGroup']);
          item.accGroup = accGroup?.code || 1;

          let classification = this.classifications.find((e) => e.value == curr?.['classification']);
  ...
  importTaiKhoanCT1(event) {
    ...
    fileReader.onload = (e) => {
      ...
      const parentCode = this.codeExcelDetail ? `${this.codeExcel}:${this.codeExcelDetail}` : this.codeExcel;
      this.accountGroupService.importTaiKhoanCT1(parentCode, request).subscribe(
        (res) => {
          this.getAccounts();
          this.messageService.add({
            severity: 'success',
  ...
  importFromExcelTaiKhoanArising(event) {
    ...
    fileReader.onload = (e) => {
      ...
      this.accountGroupService.importFromExcelTaiKhoanArising(request).subscribe(
        (res) => {
          this.messageService.add({
            severity: 'success',
            detail: AppUtil.translate(this.translateService, 'success.create'),
  ...
  exportExcel(isExportAll: boolean = false) {
    ...
    let params: ImportExportAcccountQueryParam = {
      code: parentCode,
      Loai: this.currentAccountType.value,
      isExportAll: isExportAll,
    };

    if (this.currentAccountDetail1s != null) {
      params.warehouseCode = this.currentAccountDetail1s.warehouseCode;
    }

    this.accountGroupService.exportTaiKhoanNoiBoChiTiet1(params).subscribe((res) => {
      this.openDownloadFile(res.data, 'excel');
    });
  }

  private getLookupValues() {
    this.lookupValuesService.lookupValues({ scope: LookupValueScopes.ChartOfAccount_AccGroup }).subscribe(
      (res) => {
        this.accGroups = res || [];
      },
      (err) => {
    ...
    this.menuActionItems = [
      ...
      },
      {
        ...
        command: () => {
          ...
          this.accountGroupService.exportTaiKhoan(accountType).subscribe((res) => {
            this.openDownloadFile(res.data, 'excel');
          });
        },
      },
  ...
  private openDownloadFile(_fileName: string, _ft: string) {
    try {
      var _l = this.accountGroupService.getFolderPathDownload(_fileName, _ft);
  ...
  private getAccounts() {
    this.accounts = null;
    this.$accounts = this.accountGroupService.getListDetail({}).subscribe((response: any) => {
