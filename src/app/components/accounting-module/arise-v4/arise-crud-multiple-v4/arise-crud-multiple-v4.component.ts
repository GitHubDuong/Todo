...
import { AriseCrudStockComponent } from '../../arise-v3/components/arise-crud-stock/arise-crud-stock.component';
import { AriseCrudImportProductComponent } from '../../arise-v3/components/arise-crud-import-product/arise-crud-import-product.component';
import { AriseCrudTaxComponent } from '../../arise-v3/components/arise-crud-tax/arise-crud-tax.component';
import { AriseCrudDeliveryComponent } from '../../arise-v3/components/arise-crud-delivery/arise-crud-delivery.component';
import { AriseCrudVatComponent } from '../../arise-v3/components/arise-crud-vat/arise-crud-vat.component';

@Component({
  selector: 'app-arise-crud-multiple-v4',
  templateUrl: './arise-crud-multiple-v4.component.html',
  styleUrls: ['./arise-crud-multiple-v4.component.scss'],
})
export class AriseCrudMultipleV4Component implements OnInit, AfterViewInit {
  ...
  validateForm(): boolean {
    if (this.ariseCrudProductComponent.ledgerTableForm.invalid) return false;
    // duyển mảng
    for (
      let index = 0;
      ...
      if (
        ...
        !this.ariseCrudProductComponent.isDebitDetailCodeSecondHas(index)
      ) {
        return false;
      }
      if (
        ...
        !this.ariseCrudProductComponent.isCreditDetailCodeSecondHas(index)
      )
        return false;
      ...
      if (
        ...
        !this.ariseCrudProductComponent.isCreditDetailCodeFirstHas(index)
      )
        return false;
  ...
  onF10(data: any) {
    ...
    const totalProduct = this.ariseCrudProductComponent.ledgers.controls.reduce(
    ...
    if (this.activeTabId == '0') {
      const amount = (Number(total || 0) * Number(data?.percent || 0)) / 100;
      data.amount = amount;
      const lastIndex =
        this.ariseCrudProductComponent.ledgers.controls.length - 1;
      let lastData =
        this.ariseCrudProductComponent.ledgers.controls[lastIndex].value;
      if (data.credit) {
      ...
      const lastDataTab1 =
        this.ariseCrudVatComponent.ledgers.controls[lastIndexTab1].value;

      const lastIndexTab0 =
        this.ariseCrudProductComponent.ledgers.controls.length - 1;
      let lastDataTab0 =
        this.ariseCrudProductComponent.ledgers.controls[lastIndexTab0].value;
  ...
  syncDataForStockComponent(ledgers: any[]) {
    const ids = this.resetLedgersArray(this.ariseCrudStockComponent.ledgers);
    const totalDelivery =
    ...
    ledgers.forEach((item, index) => {
      ...
      obj.percentTransport =
        Number.parseFloat(((amount / totalProduct) * 100)?.toFixed(2)) ?? 0;
      obj.amountTransport = ((obj.percentTransport ?? 0) * totalDelivery) / 100;
  ...
  onF8() {
    ...
    for (let index = 0; index < ariseProductLedgers.length; index++) {
      ...
      if (
        ...
        ariseProductLedger.percentImportTax = ariseTaxLedger.percentImportTax;
      ...
      if (
        ...
        ariseProductLedger.percentTransport = ariseStockLedger.percentTransport;
        ariseProductLedger.amountTransport = ariseStockLedger.amountTransport;
  ...
  onActionRowProduct(event) {
    if (event) {
      switch (event.action) {
        case 'remove':
          this.ariseCrudProductComponent.ledgers.removeAt(event.index);
          this.ariseCrudTaxComponent.ledgers.removeAt(event.index);
          this.ariseCrudStockComponent.ledgers.removeAt(event.index);
          break;
        case 'clear':
          this.ariseCrudProductComponent.ledgers.controls[event.index].reset();
          this.ariseCrudTaxComponent.ledgers.controls[event.index].reset();
          this.ariseCrudStockComponent.ledgers.controls[event.index].reset();
