...
export class AriseListV4Component implements OnInit, OnDestroy {
  ...
  onDelete(id, isMulti = false) {
    ...
    this.confirmationService.confirm({
      ...
      accept: () => {
        this.ledgerService
          .deleteLedger({ ids: idsTemp.join(',') }, this.dataTable.isInternal)
  ...
  onPrint(ledger) {
    return this.ledgerService
      ...
      .subscribe((res) => {
        ...
        if (
          ['PT', 'PC', 'NK', 'XK', 'CK', 'PNPC', 'PXN', 'BH'].includes(
            ledger.type,
          )
        ) {
          setTimeout(() => {
  ...
  private getDebitCreditCodePrint() {
    ...
    let ledgersPrint = this.dataTable.datas.filter(
      (x) => x.orginalVoucherNumber === this.dataPrint.orginalVoucherNumber,
    );
  ...
  private getDetail() {
    ...
    return this.ledgerService
      .getLedgerV2(this.selectAriseContextMenu.id, this.dataTable.isInternal)
      .subscribe((res) => {
        const dataTable = {
  ...
  private getDocumentTypeList() {
    this.documentService.getAllActiveDocumentV2().subscribe((resp) => {
      if (resp?.data) {
        ...
        this.dataTable.document = _.cloneDeep(
          this.documentList[this.documentList.length > 1 ? 1 : 0],
        );
        this.getLedgers(this.dataTable, (data) => {
          this.onFilterTable.emit(data);
        });
