...
import { AriseCrudMultipleV4Component } from '../arise-crud-multiple-v4/arise-crud-multiple-v4.component';
import { PayerService } from '../../../../service/payer.service';

@Component({
  selector: 'app-arise-crud-v4',
  templateUrl: './arise-crud-v4.component.html',
  styleUrls: ['./arise-crud-v4.component.scss'],
})
export class AriseCrudV4Component implements OnInit, OnDestroy {
  ...
  ariseCrudMultipleV3Component!: AriseCrudMultipleV4Component;
      ...
      const input = Object.assign(
        {},
        {
          ...
          orginalDescription:
            ...
            '',
          invoiceCode:
            item['invoiceCode']?.code || item['invoiceTaxCode'] || '',
          invoiceTaxCode:
            item['invoiceTaxCode']?.taxCode || item['invoiceTaxCode'] || '',
          ...
          orginalBookDate: AppUtil.formatLocalTimezone(
            moment(
              item['orginalBookDate'] ? item['orginalBookDate'] : new Date(),
              AppConstant.FORMAT_DATE.VN_DATE_PIPE_SHORT_DATE,
            ).format(AppConstant.FORMAT_DATE.T_DATE),
          ),
          ...
          invoiceDate: AppUtil.formatLocalTimezone(
            moment(
              item['invoiceDate'] ? item['invoiceDate'] : new Date(),
              AppConstant.FORMAT_DATE.VN_DATE_PIPE_SHORT_DATE,
            ).format(AppConstant.FORMAT_DATE.T_DATE),
          ),
        },
  ...
  filterInvoiceCode(event) {
    ...
    const list = _.filter(this.invoiceCodeList, (item) => {
      return (
        item.name &&
        item.name != '' &&
        (item.code.toLowerCase().includes(event.query.toLowerCase()) ||
          item.name.toLowerCase().includes(event.query.toLowerCase()))
      );
    });
  ...
  filterCreditDetailCodeSecond(event) {
    ...
    var callBack = () => {
      setTimeout(() => {
        ...
        if (autocompletePanel && this.creditDetailCodeSecondFilter.length > 0) {
          this.renderer.listen(autocompletePanel, 'scroll', (event) => {
            if (
              event.target.scrollHeight - event.target.clientHeight ===
              event.target.scrollTop
  ...
  private setAllowForeignCurrency() {
    this.isForeignCurrency =
      (this.isDebitCodeHas && this.fc['debitCode'].value.isForeignCurrency) ||
      (this.isCreditCodeHas && this.fc['creditCode'].value.isForeignCurrency);
