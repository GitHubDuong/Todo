...
export class InstanceBillComponent implements OnInit {
  ...
  getInvoices(event?: any): void {
    this.param.page = Math.floor((event?.first || 0) / (event?.rows || 1)) + 1;
  ...
  onRowEditDelete(item): void {
    this.invoiceDeclarationService.deleteInvoiceDeclaration(item.id).subscribe(
      (res) => {
  ...
  onRetsetData(item: any, ri: any) {
    this.invoiceDeclarationService.resetInvoiceDeclaration(item.id).subscribe(
      (res) => {
        this.messageService.add({
          severity: 'success',
          detail: AppUtil.translate(this.translateService, 'success.update'),
