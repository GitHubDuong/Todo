...
export class ToolsFixedAssetsFormComponent implements OnInit {
  ...
  onSubmit() {
    ...
    newData.buyDate = newData.buyDate
      ? moment(newData.buyDate).format(this.appConstant.FORMAT_DATE.T_DATE)
      : newData.buyDate;
    newData.usedDate = newData.usedDate
      ? moment(newData.usedDate).format(this.appConstant.FORMAT_DATE.T_DATE)
      : newData.usedDate;

    if (this.isEdit) {
      const api = this.activatedRoute.snapshot.data.isPageUse
        ? this.assetsFixed242Service.fixedAssets242Update(newData.id, newData)
