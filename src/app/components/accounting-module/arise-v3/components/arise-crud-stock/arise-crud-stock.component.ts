...
@Component({
  selector: 'app-arise-crud-stock',
  templateUrl: './arise-crud-stock.component.html',
  styleUrls: ['./arise-crud-stock.component.scss'],
})
export class AriseCrudStockComponent implements OnInit {
  ...
  onChangePercentageStock(event, index: number) {
    const controls = this.getControlsByIndex(index);
    const group = this.ledgers.controls[index];

    const currentPercentage = isNaN(Number(event?.target?.ariaValueNow))
      ? 0
      : Number(event?.target?.ariaValueNow || 0);
    const amountImportWarehouse = controls['amountImportWarehouse'].value || 0;
    const sumPercentage = Number.parseInt(
      this.ledgers.controls
        .filter((x, i) => i != index)
        .reduce((accumulator, group: any) => {
          return accumulator + group.controls['percentTransport']?.value;
        }, 0),
    );
    const total = sumPercentage + currentPercentage;
    if (total > 100) {
      controls['percentTransport'].patchValue(100 - sumPercentage);
    }
    const amountTransport =
      (Number(amountImportWarehouse || 0) * currentPercentage) / 100;
    group.patchValue({
  ...
  onChangeAmountTransport(event, index) {
    let controls = this.getControlsByIndex(index);
  ...
  onChangeAmountImportWarehouse(event, index) {
    ...
    const amountImportWarehouse = isNaN(Number(event?.target?.ariaValueNow))
      ? 0
      : Number(event?.target?.ariaValueNow || 0);
    const percentage = controls['percentTransport'].value || 0;
    // số lượng nhân đơn giá
    const amountTransport =
      (Number(amountImportWarehouse || 0) * percentage) / 100;
    group.patchValue({
      amountTransport: amountTransport,
    });
  }
