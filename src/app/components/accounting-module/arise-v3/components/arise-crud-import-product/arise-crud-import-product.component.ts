...
@Component({
  selector: 'app-arise-crud-import-product',
  templateUrl: './arise-crud-import-product.component.html',
  styleUrls: ['./arise-crud-import-product.component.scss'],
})
export class AriseCrudImportProductComponent implements OnInit {
  ...
  @Output() onActionRow: EventEmitter<any> = new EventEmitter<any>();
  ...
  validateForm(): boolean {
    ...
    for (let index = 0; index < this.ledgers.controls.length; index++) {
      ...
      if (
        [this.isDebitCodeHas(index), this.isCreditCodeHas(index)].includes(
          false,
        )
      )
        return false;
  ...
  buildLedgerGroup(data?: any): FormGroup {
    let ledgerForm = this.fb.group({
      ...
      unitPrice: data ? (data.unitPrice == '' ? 0 : data.unitPrice) : [''],
  ...
  addNewFormLedger(data?: any) {
    ...
    if (!data) {
      let previousData = _.cloneDeep(
        this.ledgers.value[this.ledgers.length - 1],
      );
      previousData.id = 0;
      previousData = this.onAutoRemoveValueInputByConfig(
  ...
  private onF2AutoFocusAfterSave() {
    ...
    const index = this.ledgers.length - 1;
    if (config) {
      let autoCompleteComponents = (
        this[
          `${config.configAriseBehaviour.codeData}TmpComponents`
        ] as QueryList<AutoComplete>
      )?.last;
      let inputMaskComponents = (
        this[
          `${config.configAriseBehaviour.codeData}TmpComponents`
        ] as QueryList<InputMask>
      )?.last;
      let InputNumberComponents = (
        this[
          `${config.configAriseBehaviour.codeData}TmpComponents`
        ] as QueryList<InputNumber>
      )?.last;
      if (autoCompleteComponents instanceof AutoComplete) {
        this.focusInput(autoCompleteComponents, index);
      } else if (inputMaskComponents instanceof InputMask) {
        this.focusInput(inputMaskComponents, index);
      } else if (InputNumberComponents instanceof InputNumber) {
        this.focusInput(InputNumberComponents, index);
      }
    }
  }
  ...
  onSelectDebitCode($event, focusInput: boolean = true, index) {
    ...
    if (focusInput) {
      if (this.isDebitCodeHas(index) && this.isDebitCodeHasDetails(index)) {
  ...
  onClearDebitCode(focusInput: boolean = true, index) {
    ...
    this.setFlagCaculate(null, index);
    if (focusInput) {
      let debitCodeTmpElm = this.debitCodeTmpComponents['_results'][index];
  ...
  filterDebitDetailCodeFirst(event, index) {
    ...
    var callBack = () => {
      setTimeout(() => {
        const autocompletePanel = this.debitDetailCodeFirstTmpComponents[
          '_results'
        ][index].el.nativeElement.querySelector('.p-autocomplete-panel');
  ...
  filterDebitDetailCodeSecond(event, index) {
    ...
    var callBack = () => {
      setTimeout(() => {
        const autocompletePanel = this.debitDetailCodeSecondTmpComponents[
          '_results'
        ][index].el.nativeElement.querySelector('.p-autocomplete-panel');
  ...
  onClearCreditCode(focusInput: boolean = true, index) {
    ...
    const creditCodeTmpElm = this.creditCodeTmpComponents['_results'][index];
    if (focusInput) {
      this.focusInput(creditCodeTmpElm, index);
    }
    this.setFlagCaculate(null, index);
  ...
  filterCreditCode(event, index) {
    ...
    if (controls['creditCode'].value instanceof String) {
      ...
      this.creditCodeFilter = [(event.query = controls['creditCode'].value)];
      return;
    }
  ...
  filterCreditDetailCodeFirst(event, index) {
    ...
    var callBack = () => {
      setTimeout(() => {
        const autocompletePanel = this.creditDetailCodeFirstTmpComponents[
          '_results'
        ][index].el.nativeElement.querySelector('.p-autocomplete-panel');
  ...
  filterCreditDetailCodeSecond(event, index) {
    ...
    var callBack = () => {
      setTimeout(() => {
        const autocompletePanel = this.creditDetailCodeSecondTmpComponents[
          '_results'
        ][index].el.nativeElement.querySelector('.p-autocomplete-panel');
        if (
          autocompletePanel &&
  ...
  private getWarehouse(key: string = 'credit' || 'debit', index): any {
    ...
    if (key === 'debit') {
      ...
      if (
        ...
        return {
          code: controls['creditDetailCodeSecond'].value.warehouseCode,
          name: controls['creditDetailCodeSecond'].value.warehouseName,
        };
      }
      // Nếu tài khoản có chi tiết 1
      // Nếu chi tiết 1 không có chi tiết 2
  ...
  getDebitNameRowData(formControl: FormControl): string {
    let rowData = formControl.value;
    if (rowData == null) {
      return '';
    }

    let debitFirst = rowData.debitDetailCodeFirst;
    let debitSecond = rowData.debitDetailCodeSecond;

    let debitNames = [debitFirst?.name, debitSecond?.name].filter(
      (item) => item?.length > 0,
    );

    let debitCode = rowData.debitCode;
    let result = debitNames.join(' - ');

    if (result.length == 0) {
      result = debitCode?.name;
    }
    return result || '';
  }

  getCreditNameRowData(formControl: FormControl): string {
    let rowData = formControl.value;
    if (rowData == null) {
      return '';
    }
    let creditFirst = rowData.creditDetailCodeFirst;
    let creditSecond = rowData.creditDetailCodeSecond;

    let creditNames = [creditFirst?.name, creditSecond?.name].filter(
      (item) => item?.length > 0,
    );

    let creditCode = rowData.creditCode;
    let result = creditNames.join(' - ');

    if (result.length == 0) {
      result = creditCode?.name;
    }
