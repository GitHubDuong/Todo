...
export class RelativesFormComponent {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (this.isEdit && this.formData && Object.keys(this.formData).length > 0) {
      ...
      this.relativeForm.patchValue({
        ...
        identifyCreatedDate: moment(this.formData.identifyCreatedDate).format(
          this.appConstant.FORMAT_DATE.VN_DATE_PIPE_SHORT_DATE,
        ),
        identifyCreatedPlace: this.formData.identifyCreatedPlace,
        identifyExpiredDate: moment(this.formData.identifyExpiredDate).format(
          this.appConstant.FORMAT_DATE.VN_DATE_PIPE_SHORT_DATE,
        ),
        ...
        unionMember: this.formData.unionMember || 1,
  ...
  checkValidValidator(fieldName: string) {
    return ((this.relativeForm.controls[fieldName].dirty ||
      ...
      (this.isInvalidForm && this.relativeForm.controls[fieldName].invalid)
      ? 'ng-invalid ng-dirty'
      : '';
  }
  ...
  onSubmit() {
    ...
    let newData = this.cleanObject(cleanData(this.relativeForm.getRawValue()));
    this.onCancel.emit({});
    if (this.isEdit) {
  ...
  cleanObject(data) {
    ...
    //     '';
    newData.nativePlace =
      this.nativeProvinces.find((item) => item.id === newData.nativeProvinceId)
        ?.name || '';
  ...
  doAttachFile(event: any): void {
    ...
    this.userService.uploadFiles(image).subscribe((res) => {
      console.log(res);
      if (res && res.body) {
        this.relativeForm.controls['avatar'].setValue(res.body.imageUrl);
      }
    });
  }
