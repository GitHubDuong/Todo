...
export class RelationFormComponent implements OnInit, OnChanges, AfterViewInit {
  ...
  checkValidValidator(fieldName: string) {
    return ((this.RelationForm.controls[fieldName].dirty ||
      ...
      (this.isInvalidForm && this.RelationForm.controls[fieldName].invalid)
      ? 'ng-invalid ng-dirty'
      : '';
  }
  ...
  onSubmit() {
    ...
    this.onCancel.emit({});
    if (this.isEdit) {
      this.RelationService.updateRelation(newData, this.formData.id).subscribe(
        (res) => {
          this.onCancel.emit({});
        },
      );
  ...
  cleanObject(data) {
    ...
    if (newData?.type === 1) {
      ...
      newData.personOppositeName = newData.personOppositeId
        ? this.userLists.find((item) => item.id === newData.personOppositeId)
            ?.fullName
        : '';
    }
