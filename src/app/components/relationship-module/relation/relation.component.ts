...
export class RelationComponent implements OnInit {
  ...
  @ViewChild('RelationForm') RelationFormComponent: RelationFormComponent | undefined;
  ...
  getRelation(event?: any, isExport: boolean = false): void {
    ...
    this.pendingRequest = this.relationService.getListRelation(this.getParams).subscribe((response: TypeData<Relation>) => {
      AppUtil.scrollToTop();
  ...
  getDetail(RelationId) {
    this.relationService.getRelationDetail(RelationId).subscribe((response: Relation) => {
  ...
  onDelete(RelationId) {
    let message;
    this.translateService.get('question.delete_relation').subscribe((res) => {
      message = res;
    });
    this.confirmationService.confirm({
      message: message,
      accept: () => {
        this.relationService.deleteRelation(RelationId).subscribe((response: any) => {
          this.getRelation();
        });
      },
    });
