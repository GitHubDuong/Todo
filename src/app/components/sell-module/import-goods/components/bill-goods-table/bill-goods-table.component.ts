...
export class BillGoodsTableComponent implements OnInit {
  ...
  @Input() taxRates: any[] = [];
  @Input() isTransferStock: boolean = false;
  ...
  @ViewChild('taxRatesComponentBill')
  taxRatesComponentBill: ListOfTaxRatesComponent;
  ...
  onPayment(isRedirect: boolean = false) {
    if (this.isTransferStock) {
      this.billTab.data.typePay = null;
    }
    ...
    if (
      ...
      this.messageService.add({
        severity: 'info',
        detail: AppUtil.translate(this.translateService, 'info.fail_cn'),
      });
      return;
    }
  ...
  calculateDiscountPrice(product: any) {
    ...
    if (product.discountType == DiscountTypeEnum.Percent) {
      return (
        (product.cashierRequest.salePrice *
          product.cashierRequest.discountPrice) /
        100
      );
    }
  ...
  onChangeBillBox = (event: any, product: ICashierImport) => {
    ...
    const unitPrice = Number(product.cashierRequest.unitPrice || 0);
    product.cashierRequest.amount =
      Number(product.cashierRequest.quantity || 0) * unitPrice;
  ...
  getMessage = () => {
    ...
    if (this.billTab.data.vat !== undefined && this.billTab.data.vat !== 0) {
      ...
      if (this.billTab.data.invoiceTaxCode) {
        message = `${message ? `${message} - ` : ''}${this.billTab.data.invoiceTaxCode?.taxCode ||
          this.billTab.data.invoiceTaxCode
        }`;
      }
