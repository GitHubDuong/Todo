...
export class ImportGoodsTableComponent implements OnInit {
  ...
  @Output() onPhieuNhapInputed = new EventEmitter();
  ...
  referenceData: any = {};
  @Input() isShowPhieuNhap: boolean = false;
  ...
  /**
   * TODO: using pipe to improve performance
   */
  getProduceAccount(product: ICashierImport) {
    const arr: string[] = [];
    if (product.parentRef && product.parentRef.includes(':')) {
      const [account] = product.parentRef.split(':');
      account && arr.push(account);
    } else if (product.parentRef) {
      arr.push(product.parentRef);
    }
    product.warehouseCode && arr.push(product.warehouseCode);
    return arr.join(' - ');
  }
  ...
  getProductCode(product: ICashierImport): string {
    const arr: string[] = [];
    if (product.parentRef && product.parentRef.includes(':')) {
      const [, detail1] = product.parentRef.split(':');
      detail1 && arr.push(detail1);
    }
    product.code && arr.push(product.code);
    return arr.join(' - ');
  ...
  getGoodsForCashiers(event?: any, isRefresh: boolean = false): void {
    ...
    params.chartOfAccountFilterId =
      this.getParamsChartAccountFilter.chartOfAccountFilterId;
  ...
  getAllGoodsForCashiers(): void {
    this.getParams.chartOfAccountFilterId =
      this.getParamsChartAccountFilter.chartOfAccountFilterId;
    this.goodsService
      .getListChartOfAccountForCashiser(AppUtil.cleanObject(this.getParams))
      .subscribe((response: any) => {
  ...
  onChangePhieuNhap() {
    this.onPhieuNhapInputed.emit(this.referenceData);
  }
  ...
  getListCustomer(searchText: string = '') {
    this.customerService.getAllCustomer(searchText, 1).subscribe((res: any) => {
      this.listCustomers = res.data;
      this.filteredCustomers = this.listCustomers.map(
        (item) => `${item.code} | ${item.name}`,
      );
      this.cashierMediator.notifyOnCustomerFiltered(this.listCustomers);
    });
  }
