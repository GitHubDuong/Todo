...
import { Bill, BillDetail, ChangeStatusResult } from 'src/app/models/cashier.model';
...
import { BillNotificationComponent } from '../cashier/components/bill-notification/bill-notification.component';
...
export class OrderComponent implements OnInit {
  ...
  getBillIdNewest() {
    this.billService.getBillIdNewestByType(this.billTypeSelected).subscribe((res) => {
      this.newestBillNumberModel = res;
    });
  }
  ...
  getDescriptionForLedger() {
    ...
    return customerName.length > 0 ? `Bán hàng cho ${this.selectedCustomer.name}` : 'Bán hàng Online';
  }
  ...
  async addBill() {
    if (this.tableCode == null || this.tableCode == undefined) {
      return;
    }
    const desk: any = this.desks.find((x) => x.code === this.tableCode);
    let billTab = this.billTabs.find((x) => x.deskId === desk.id);
    if (billTab) {
    ...
    let floor = this.floors.find((x) => x.id === desk.floorId);
  ...
  async addProduct(event) {
    let doAddProduct = async () => {
      if (this.desks && this.desks.length === 1 && this.floors && this.floors.length === 1 && this.billTabs.length === 1) {
  ...
  private addNewBill(desk: RoomTable, floor: RoomTable): any {
    return {
      ...
      data: {
        ...
        floorName: '',
        customerNumber: 1,
        customerId: !AppUtil.isEmpty(this.selectedCustomer) ? this.selectedCustomer.id : 0,
        customerName: !AppUtil.isEmpty(this.selectedCustomer) ? `${this.selectedCustomer.code} | ${this.selectedCustomer.name}` : '',
        customerNameOnly: !AppUtil.isEmpty(this.selectedCustomer) ? `${this.selectedCustomer.name}` : '',
        customerTaxCode: !AppUtil.isEmpty(this.selectedCustomer) ? `${this.selectedCustomer.taxCode}` : '',
        discountType: 'money',
        totalAmount: 0,
        discountPrice: 0,
        note: '',
        ...
        vatAmount: 0,
        vatCode: '',
        debitCode: !AppUtil.isEmpty(this.selectedCustomer) ? this.selectedCustomer.debit?.code : '',
        debitDetailCodeFirst: !AppUtil.isEmpty(this.selectedCustomer) ? this.selectedCustomer.debitDetailFirst?.code : '',
        debitDetailCodeSecond: !AppUtil.isEmpty(this.selectedCustomer) ? this.selectedCustomer.debitDetailSecond?.code : '',
      },
    };
  }
  ...
  onAddProduct(product) {
    ...
    this.selectedBillTab.data.products = [...products, Object.assign({}, product)];
  }
  ...
  onEditSplitMerge() {
    // check exist bill
    if (this.desks.find((x) => x.id === this.mergeBillTab.mergeDeskId && x.isChoose)) {
      this.messageService.add({
        severity: 'info',
        detail: AppUtil.translate(this.translateService, 'Bàn đã tạo hóa đơn'),
      });
      return;
    }
    // check info merge type
    if (!this.mergeBillTab.splitMergeType || !this.mergeBillTab.mergeFloorId || !this.mergeBillTab.mergeDeskId) {
      this.messageService.add({
        severity: 'info',
        detail: AppUtil.translate(this.translateService, 'Thông tin lầu bàn chưa được chọn'),
      });
      return;
    }
    // check split exist desk
    if (this.mergeBillTab.splitMergeType === 'split' && this.billTabs.find((x) => x.deskId === this.mergeBillTab.mergeDeskId)) {
      this.messageService.add({
        severity: 'info',
        detail: AppUtil.translate(this.translateService, 'Bàn đã được chọn trong hệ thống hoặc ds hóa đơn'),
      });
      return;
    }
    ...
    if (this.mergeGoods.filter((x) => x.checked).length === 0) {
      this.messageService.add({
        severity: 'info',
        detail: AppUtil.translate(this.translateService, 'Không có hàng hóa nào được chọn'),
      });
      return;
    }
    // check quantity merge goods
    if (this.mergeGoods.filter((x) => x.checked && !x.mergeQuantity).length > 0) {
      this.messageService.add({
        severity: 'info',
        detail: AppUtil.translate(this.translateService, 'Số lượng hàng hóa không trống'),
      });
      return;
    }
    ...
    if (this.mergeBillTab.splitMergeType === 'split') {
      ...
      let newBill = {
        ...
        deskName: this.desks.find((x) => x.id === this.mergeBillTab.mergeDeskId).name,
        floorId: this.mergeBillTab.mergeFloorId,
        floorName: this.floors.find((x) => x.id === this.mergeBillTab.mergeFloorId).name,
        title: `Bill ${this.newestBillNumberModel.billNumber}`,
        data: {
          id: this.newestBillNumberModel.billOrder,
          isRealId: false,
      ...
      tempGoods.forEach((tempGoods) => {
        let oldGoods = this.selectedBillTab.data.products.find((x) => x.id === tempGoods.id);
        oldGoods.billQuantity = oldGoods.billQuantity - tempGoods.billQuantity;
      });
      if (!this.selectedBillTab.data.products.find((x) => x.billQuantity > 0)) {
      ...
      let oldProducts = this.billTabs.find((x) => x.tabId === this.selectedBillTabId).data.products;
      let newProducts = this.billTabs.find((x) => x.deskId === this.mergeBillTab.mergeDeskId).data.products;
      ...
      tempGoods.forEach((tempGoods) => {
        ...
        if (newGoods) {
          newGoods.billQuantity = newGoods.billQuantity + tempGoods.billQuantity;
        } else {
      ...
      this.setSelectedTab(this.billTabs.find((x) => x.deskId === this.mergeBillTab.mergeDeskId).tabId);
    }
    this.mergeGoods = [];
  ...
  getLastSurcharge(loadBill?) {
    this.surchargesService.getLastSurcharge().subscribe((res: any) => {
      ...
      if (loadBill) {
        ...
        this.selectedBillTab.data.amountReceivedByCus = this.getDiscountBillMoney();
      }
    });
  }
  ...
  private async saveBill(param: Bill, isPayment) {
    ...
    if (params.id === 0) {
      ...
      if (this.typeSave === 'saveTemp') {
        this.billService.updateBill(params, params.id).subscribe((res: any) => {
          this.onSaveBillDetail(res.data.id);
          this.messageService.add({
            severity: 'success',
  ...
  updateSurchargeBill(param: Bill) {
    if (this.surchargeData != null && this.surchargeData.value > 0) {
      ...
      this.billService.updateBillSurcharge(newSurChargeBill, params.id).subscribe((res: any) => {
        this.messageService.add({
          severity: 'success',
          detail: AppUtil.translate(this.translateService, 'success.update'),
        });
  ...
  doSendBill(isPayment: boolean = false) {
    ...
    if (isPayment && this.selectedBillTab.data && this.selectedBillTab.data.isRealId) {
      this.onCompleteBill();
      return;
    }
  ...
  private createBillDetail(param: BillDetail[]) {
    this.billDetailService.createBillDetail(param).subscribe(
      async (res: any) => {
  ...
  checkSetSelectedBillTab(tabId?) {
    ...
    if (this.appUtil.isEmpty(this.selectedBillTab)) {
      this.selectedBillTab = this.billTabs.find((x) => x.tabId === this.selectedBillTabId);
    }
  }
  ...
  getBillReq(isPayment: boolean) {
    ...
    return {
      ...
      amountSendToCus: selectedBill.amountReceivedByCus - this.getDiscountBillMoney() || 0,
  ...
  onCompleteBill() {
    ...
    this.billService.completeBill(this.selectedBillTab.data.id, params).subscribe((res: any) => {
      if (res && res.status == 400) {
        this.messageService.add({
          severity: 'info',
          detail: AppUtil.translate(this.translateService, `${res.message}`),
        });
        return;
      }
      if (this.isPayment || (this.typeSave === 'saveTemp' && this.typePrint === 'XK')) {
      ...
      this.messageService.add({
        severity: 'success',
        detail: AppUtil.translate(this.translateService, `${this.selectedUser} đã hoàn thành hóa đơn`),
      });
      this.billNotification.getNotificationMessage();
      this.selectedBillTab.data.status = res.data.status;
    });
  ...
  getDiscountBillMoney() {
    ...
    if (this.selectedBillTab.data.discountType === 'percent') {
      totalPrice =
        this.selectedBillTab.data.totalAmount - (this.selectedBillTab.data.totalAmount / 100) * this.selectedBillTab.data.discountPrice;
    } else {
      totalPrice = this.selectedBillTab.data.totalAmount - this.selectedBillTab.data.discountPrice;
    }
  ...
  onBillTypeChanged() {
    this.billService.changeBillNumberByType(this.selectedBillTab?.data.id, this.billTypeSelected).subscribe((res) => {
  ...
  openFile() {
    ...
    window.open(`${environment.serverURL}/api/ReportDownload/download-file-template?fileName=${fileUrl}`);
  }

  GetDataPrintLedger(billTab) {
    // call API
    this.billService.GetLedgerFromBillId(billTab.data.id).subscribe((res: any) => {
      let data = res[0];
      this.dataPrint = {
        ...data,
  ...
  onUpdateAccountant(event) {
    const tabId = event.tabId;
    const debitCode = event.debitCode;
    const debitDetailCodeFirst = event.debitDetailCodeFirst;
    this.billTabs.forEach((tab) => {
      if (tab.tabId == tabId) {
        tab.data.debitCode = debitCode;
        tab.data.debitDetailCodeFirst = debitDetailCodeFirst;
      }
    });
    this.goodsTable.onCustomerNameSelect(`${this.selectedCustomer.code} | ${this.selectedCustomer.name}`);
  }
}
