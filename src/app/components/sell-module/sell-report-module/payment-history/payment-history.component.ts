...
export class PaymentHistoryComponent implements OnInit {
  ...
  onLoadDeskFloor() {
    this.roomTableService
      ...
      .subscribe((deskFloors: TypeData<RoomTable>) => {
        this.lstFloors = deskFloors.data.filter((x) => x.isDesk === false);
        this.lstDesks = deskFloors.data.filter((x) => x.isDesk === true);
      });
  }
  ...
  getBillDetails(
    ...
    this.billDetailService
      ...
      .subscribe((res: TypeData<BillDetail>) => {
        ...
        let temp: any;
        temp = this.lstFloors.find((x) => x.id === bill.floorId);
        this.dataPrint.floorName = temp ? temp.name : '';
        temp = this.lstDesks.find((x) => x.id === bill.deskId);
        this.dataPrint.deskName = temp ? temp.name : '';
        temp = this.employees.find((x) => x.username === bill.userCode);
        this.dataPrint.username = temp ? temp.fullName : '';
        if (isXuatKho == false) this.doPrintBill(bill);
        else {
          this.dataPrint.goods = res.data;
          this.dataPrint.discountPriceBill = res.data[0]?.discountPriceBill;
      ...
      setTimeout(() => {
        if (window) {
          ...
          bill.billDetails.forEach((billDetail: BillDetail) => {
            let goods = this.lstGoods.find((x) => x.id === billDetail.goodsId);
            products +=
              '    \t\t\t\t\t\t\t<tr>\n' +
              '    \t\t\t\t\t\t\t\t<td>' +
              ...
              (billDetail.discountType === 'percent' ? '%' : 'đ') +
              '</td>\n' +
              "    \t\t\t\t\t\t\t\t<td style='text-align: right;'>" +
              this.formatMoney(billDetail.quantity * billDetail.unitPrice) +
              'đ</td>\n' +
              '    \t\t\t\t\t\t\t</tr>\n' +
              '                                <tr>\n';
          });
          if (navigator.userAgent.toLowerCase().indexOf('chrome') > -1) {
            ...
            popup.document.write(
              '<!DOCTYPE html><html><head>  ' +
                ...
                ' <br />\n' +
                '    \t\t\t\tBằng chữ: <span>' +
                this.appUtil.formatCurrencyVNDString(bill.totalAmount) +
                '.</span>' +
                ' <br />\n' +
  ...
  editSoLuong(bill) {
      ...
      (item) => {
        return (
          _.findIndex(bill.billDetailsRoot, (root: any) => {
            return root.id == item.id && item.quantity != root.quantity;
          }) != -1
        );
      },
    );
    ...
    this.billDetailService.saveRefundGoods(billId, inputs).subscribe((res) => {
  ...
  onAdjustBillDetail(billId: number) {
    this.billDetailService
      .getBillDetailAndPromotions(billId)
      .subscribe((res: TypeData<BillDetail>) => {
