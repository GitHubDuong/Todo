  ...
  startOnShift() {
    this.tillService.startOfShift({}).subscribe((res) => {
      ...
      this.messageService.add({
        severity: 'success',
        detail: AppUtil.translate(this.translateService, 'Bắt đầu vào ca'),
      });
    });
  }
      ...
      if (window) {
        ...
        billTab.data.products.forEach((product) => {
          ...
          products +=
            ...
            "    \t\t\t\t\t\t\t\t<td style='text-align: right;'>" +
            this.formatMoney(
              this.getDiscountMoney(product, product.discountPrice),
            ) +
            ' = ' +
            ' </td>\n' +
            "    \t\t\t\t\t\t\t\t<td style='text-align: right;'>" +
        ...
        if (navigator.userAgent.toLowerCase().indexOf('chrome') > -1) {
          ...
          popup.document.write(
            '<!DOCTYPE html><html><head>  ' +
              ...
              this.formatMoney(this.selectedBillTab.data.discountPrice) +
              (this.selectedBillTab.data.discountType === 'percent'
                ? '%'
                : 'đ') +
              '.</span>' +
              ' <br />\n' +
              '    \t\t\t\tTổng tiền: <span>' +
              this.formatMoney(this.selectedBillTab.data.totalAmount) +
              'đ.</span>' +
              ' <br />\n' +
              '    \t\t\t\tBằng chữ: <span>' +
              ...
              this.formatMoney(this.selectedBillTab.data.amountReceivedByCus) +
              'đ.</span>' +
              ' <br />\n' +
