...
export class ImportStockDetailComponent implements OnInit {
  ...
  onVatProduct(product) {
    ...
    this.selectedProduct.taxVatRate =
      (this.selectedProduct.taxVat / this.selectedProduct.salePrice) * 100;
    if (!this.selectedProduct.discountType) {
  ...
  filterCustomerName(event) {
    ...
    for (let i = 0; i < this.customers.length; i++) {
      if (
        this.customers[i].name.toLowerCase().includes(event.query.toLowerCase())
      ) {
        filtered.push(`${this.customers[i].code} | ${this.customers[i].name}`);
      }
    }
  ...
  onPayment() {
    if (
      ...
      (!this.billTab.data.debitCode || !this.billTab.data.debitDetailCodeFirst)
    ) {
      this.messageService.add({
        severity: 'info',
    ...
    this.billDetailService
      ...
      .pipe(
        mergeMap((response: TypeData<BillDetailPrint>) => {
          ...
          this.dataPrint.surchargeBill = response.data[0]?.surchargeBill;
