...
@Component({
  selector: 'app-produce-planning-warehouse',
  templateUrl: './produce-planning-warehouse.component.html',
  styleUrls: ['produce-planning-warehouse.component.scss'],
})
export class ProducePlanningWarehouseComponent implements OnInit {
  mode = 'website';
  isInvalidForm = false;
  ...
  constructor(
    ...
    private readonly producePlanningWarehouse: ProducePlanningWarehouseService,
  ...
  getOrder(event?: any, isExport: boolean = false) {
    ...
    this.producePlanningWarehouse.getPaging(this.getParams).subscribe({
      next: (res) => {
  ...
  getDetail(bill: any): void {
    ...
    this.loading = true;
    this.producePlanningWarehouse.getDetail(bill.id).subscribe({
      next: (res) => {
        this.display = true;
        this.loading = false;
        if (res.items) {
  ...
  onOrder(billDetail: any[]): void {
    ...
    if (this.selectProductionOrder) {
      this.producePlanningWarehouse.update(this.paramCreateProduceProducts).subscribe({
        next: (res): void => {
          this.loading = false;
    ...
    this.producePlanningWarehouse.create(this.paramCreateProduceProducts).subscribe({
      next: (res): void => {
        this.loading = false;
        this.displayBillDetail = false;
        this.messageService.add({
  ...
  onUpdateOrder(order) {
    this.producePlanningWarehouse.update(order).subscribe(
      (res) => {
        this.messageService.add({
          severity: 'success',
  ...
  onDetailProduce(item) {
    this.loading = true;
    this.producePlanningWarehouse.getDetail(item.id).subscribe(
      (res) => {
        this.loading = false;
  ...
  accept(orderId: number): void {
    this.producePlanningWarehouse.accept(orderId).subscribe((res: any) => {
      AppUtil.scrollToTop();
      this.messageService.add({
        severity: 'success',
  ...
  notAccept(orderId: number): void {
    this.producePlanningWarehouse.notAccept(orderId).subscribe((res: any) => {
      AppUtil.scrollToTop();
      this.messageService.add({
        severity: 'success',
  ...
  delete(orderId: number): void {
    this.producePlanningWarehouse.delete(orderId).subscribe((res: any) => {
      AppUtil.scrollToTop();
      this.messageService.add({
        severity: 'success',
