...
import { MenuOfGoodsFormComponent } from '../setup-module/menu-of-goods/component/menu-of-goods-form/menu-of-goods-form.component';
...
export class PriceListComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.typeCategory = AppUtil.getCategoryTypes();
    this.getTaxRate();
  }
  getTaxRate() {
  ...
  onAddCategory() {
    ...
    this.categoryForm.onChangeDefaultPriceList();
    this.displayCategoryForm = true;
  }
  ...
  getDetail1(accountCode: string) {
    const params = this.getParams.warehouse != null
      ? { warehouseCode: this.getParams.warehouse }
      : {};

    this.chartOfAccountService.getDetail(accountCode, params).subscribe((res: any) => {
  ...
  import(evt) {
    ...
    reader.onload = (e: any) => {
        ...
        datas.forEach((element) => {
          ...
          objProps.forEach((item, index) => {
            if (item == 'priceList') {
              ...
              objItem[item] = parseFloat(element[index + 2]);
            } else {
      ...
      this.goodsServices.importExcelListOfGoods(dataImport, false).subscribe(
        (res) => {
          this.getGoods();
          this.messageService.add({
            severity: 'success',
  ...
  checkGoodNew() {
    this.goodsServices.checkGoodNew().subscribe((res) => {
      this.isChekNewGood = res.data;
    });
  }
