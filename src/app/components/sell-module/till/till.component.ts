...
import { PageFilterCustomer } from 'src/app/service/customer.service';
...
export class TillComponent implements OnInit {
  ...
  getTills(event?: any, isExport: boolean = false): void {
    ...
    this.pendingRequest = this.tillsService.getTills(this.getParams).subscribe((response: TypeData<SurchargeModel>) => {
      AppUtil.scrollToTop();
  ...
  calculateMoneyAfterEndOfShift(till: any) {
    return (till.fromAmount || 0) + (till.toAmountAuto || 0) - (till.amountDifferent || 0);
  }
  ...
  onDelete(surchargeId) {
    this.tillsService.deleteTill(surchargeId).subscribe((response: SurchargeModel) => {
