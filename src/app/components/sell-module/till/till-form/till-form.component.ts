...
export class TillFormComponent implements OnInit {
  ...
  checkValidMultiValidator(fieldNames: string[]) {
    for (let i = 0; i < fieldNames.length; i++) {
      if (
        ...
        (this.isInvalidForm && this.tillForm.controls[fieldNames[i]].invalid)
      ) {
        return true;
      }
    }
  ...
  onSubmit() {
    ...
    if (!this.isFinish) {
      this.tillService
        .startOfShift({ fromAmount: this.tillForm.value.fromAmount })
        .subscribe((res) => {
    ...
    let newData = this.cleanObject(AppUtil.cleanObject(this.tillForm.value));
