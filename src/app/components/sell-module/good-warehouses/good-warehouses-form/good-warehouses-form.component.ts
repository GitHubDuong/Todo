...
export class GoodWarehousesFormComponent implements OnInit {
  ...
  getDetail(id) {
    ...
    this.goodWarehouseService.getDetail(id).subscribe((res: any) => {
      this.goodsForm.setValue({
        ...res,
        dateExpiration:
          res.dateExpiration != null ? new Date(res.dateExpiration) : null,
        dateManufacture:
          res.dateManufacture != null ? new Date(res.dateManufacture) : null,
        priceList: [],
  ...
  onSubmit() {
    ...
    newData.dateManufacture = AppUtil.adjustDateOffset(newData.dateManufacture);
    newData.dateExpiration = AppUtil.adjustDateOffset(newData.dateExpiration);
  ...
  getChartOfAccountDetails(accountCode, type, isInit = false) {
    if (type === 'debit2') {
      this.selectedDebit2 = this.getCreditDebitObject(accountCode, 'debit2');
    ...
    this.chartOfAccountService.getDetail(accountCode).subscribe((res: any) => {
      ...
      switch (type) {
        case 'debit':
          ...
          this.selectedDebit = this.getCreditDebitObject(accountCode, 'debit');
          if (this.selectedDebit && this.selectedDebit.warehouseCode) {
            this.goodsForm.controls['warehouse'].setValue(
        ...
        case 'debit1':
          ...
          if (this.selectedDebit1 && this.selectedDebit1.warehouseCode) {
            this.goodsForm.controls['warehouse'].setValue(
  ...
  onDebitSelect(event) {
    if (event.includes('|')) {
      this.goodsForm.controls['account'].setValue(event.split('|')[0].trim());
      this.onChangeCreditDebit({ value: event.split('|')[0].trim() }, 'debit');
    }
  }
  ...
  onDebit1Select(event) {
    if (event.includes('|')) {
      this.goodsForm.controls['detail1'].setValue(event.split('|')[0].trim());
      this.onChangeCreditDebit(
        {
          value: `${this.selectedDebit.code}:${event.split('|')[0].trim()}`,
        },
        'debit1',
      );
    }
  ...
  filterDebit1Name(event) {
    ...
    for (let i = 0; i < this.debits1.length; i++) {
      if (this.debits1[i].code.toLowerCase().includes(query.toLowerCase())) {
        filtered.push(`${this.debits1[i].code} | ${this.debits1[i].name}`);
      }
    }
  ...
  onDebit2Select(event) {
    if (event.includes('|')) {
      this.goodsForm.controls['detail2'].setValue(event.split('|')[0].trim());
      this.onChangeCreditDebit({ value: event.split('|')[0].trim() }, 'debit2');
    }
  }
  ...
  filterDebit2Name(event) {
    ...
    for (let i = 0; i < this.debits2.length; i++) {
      if (this.debits2[i].code.toLowerCase().includes(query.toLowerCase())) {
        filtered.push(`${this.debits2[i].code} | ${this.debits2[i].name}`);
      }
    }
