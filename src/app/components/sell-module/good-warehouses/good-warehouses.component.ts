...
import { GoodWarehousesFormComponent } from './good-warehouses-form/good-warehouses-form.component';

@Component({
  selector: 'app-good-warehouses',
  templateUrl: './good-warehouses.component.html',
  ...
  styles: [
    `
      ...
      ::ng-deep tr.p-selectable-row.good-printed {
        background: #bfffe2;
      }
    `,
  ],
})
export class GoodWarehousesComponent implements OnInit {
  ...
  goodWarehousesForm: GoodWarehousesFormComponent;
  loading: boolean = true;

  sortFields: any[] = [];
  sortTypes: any[] = [];
  defaultLogoURL: string = 'Uploads\\Images\\logo-02.png';
  first = 0;
  ...
  getGoods(event?: any, isExport: boolean = false): void {
    ...
    this.pendingRequest = this.goodWarehouseService
  ...
  goodsSync() {
    this.goodWarehouseService.syncChartOfAccount().subscribe(
      (res) => {
        this.getGoods();
