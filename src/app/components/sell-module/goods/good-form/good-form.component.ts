...
import { AccountType, AddAccountDetailType, } from '@app/components/accounting-module/account-v2/account.model';
...
export class GoodFormComponent extends BaseAccountComponent implements OnInit {
  ...
  getDetail(id) {
    ...
    this.goodsService.getDetail(id).subscribe((res: any) => {
      ...
      if (res.accountObj) {
        res.accountObj.displaySelected =
          res.accountObj.code + ' - ' + res.accountObj.name;
      }
      if (res.detailFirstObj) {
        res.detailFirstObj.displaySelected =
          res.detailFirstObj.code + ' - ' + res.detailFirstObj.name;
      }
      if (res.detailSecondObj) {
        res.detailSecondObj.displaySelected =
          res.detailSecondObj.code + ' - ' + res.detailSecondObj.name;
      }
