...
import { BillGoodsTableComponent } from '../import-goods/components/bill-goods-table/bill-goods-table.component';
import { ImportGoodsTableComponent } from '../import-goods/components/import-goods-table/import-goods-table.component';
import { BillGoodsNotificationComponent } from '../import-goods/components/bill-goods-notification/bill-goods-notification.component';
import { PrintBillGoodsComponent } from '../import-goods/components/print-bill-goods/print-bill-goods.component';
...
export class TransferStockComponent implements OnInit, AfterViewInit {
  ...
  @ViewChild('printBillComponent') printBillComponent: PrintBillGoodsComponent;
  ...
  ngAfterViewInit(): void {
    // this.appBillTable.isTransferStock = true;
  }
  ...
  getCOAFilters(): void {
    this.coaFiltersService
      ...
      .subscribe((response: any) => {
        const accountDesc = response.data.sort((account1, account2) =>
          account1.order < account2.order ? -1 : 1,
        );
        this.accountDesc = accountDesc.map((account: any) => {
          return {
            key: account.name,
            value: account.id,
        ...
        this.getParams.chartOfAccountFilterId = this.accountDesc[0].value;
        this.billTypeChartOfAccount = this.accountDesc[0].type;
        this.documentCode = 'CK';
  ...
  getGoodsForCashiers() {
    const accountDesc = this.accountDesc.find(
      (item) => item.value === this.getParams.chartOfAccountFilterId,
    );
    this.documentCode = accountDesc?.documentCode;
    this.billTypeChartOfAccount = accountDesc?.type;
    if (this.documentCode) {
      this.document = this.documentList.find(
        (item) => item.code === this.documentCode,
      );
    } else {
      this.document = this.documentList.find(
        (item) => item.code === DocumentCodeWithPaymentType[this.billTypePay],
      );
    }
  ...
  setOriginalVoucherNumber = () => {
    ...
    this.orginalVoucherNumber = `${
      ...
      this.isCreate ? this.nextStt : this.oldNextStt,
      3,
    )}`.replace(' ', '');
  ...
  async getBillIdNewest() {
    this.billService
      .getBillIdNewestByType(this.billTypeSelected)
      .subscribe((res) => {
  ...
  onChangePhieuNhap(event) {
    this.referenceData = event;
  }
  ...
  onEditSplitMerge() {
    ...
    if (
      this.mergeBillTab.splitMergeType === 'split' &&
      this.billTabs.find((x) => x.deskId === this.mergeBillTab.mergeDeskId)
    ) {
      this.messageService.add({
        severity: 'info',
        detail: AppUtil.translate(
    ...
    if (
      this.mergeGoods.filter((x) => x.checked && !x.mergeQuantity).length > 0
    ) {
      this.messageService.add({
        severity: 'info',
        detail: AppUtil.translate(
    ...
    if (this.mergeBillTab.splitMergeType === 'split') {
      ...
      let newBill = {
        ...
        deskName: this.desks.find((x) => x.id === this.mergeBillTab.mergeDeskId)
          .name,
  ...
  async onShowPayment(billTab, isPayment, typeSave, typePrint: string = '') {
    const cashierRequest = {
      ...
      customerName: !AppUtil.isEmpty(this.selectedCustomer)
        ...
        : '',
      invoiceAdditionalDeclarationCode:
        billTab.data.invoiceAdditionalDeclarationCode,
      ...
      referenceBookDate: this.referenceData
        ? AppUtil.formatLocalTimezone(
            moment(
              this.referenceData.referenceBookDate
                ? this.referenceData.referenceBookDate
                : new Date(),
              AppConstant.FORMAT_DATE.VN_DATE_PIPE_SHORT_DATE,
            ).format(AppConstant.FORMAT_DATE.T_DATE),
      ...
      referenceVoucherNumber: this.referenceData
        ? this.referenceData.referenceVoucherNumber
        : billTab.data['referenceVoucherNumber'],
      referenceFullName: this.referenceData
        ? this.referenceData.referenceFullName
        : billTab.data['referenceFullName'],
      referenceAddress: this.referenceData
        ? this.referenceData.referenceAddress
        : billTab.data['referenceAddress'],
      invoiceDate: !billTab.data['invoiceDate']
        ? null
        : AppUtil.formatLocalTimezone(
            moment(
  ...
  buildCreditCode(billTab: any) {
    ...
    const payAccount = this.payTypesList.find(
      (item) => item.code === billTab.data.typePay,
    );
    if (this.billTypeChartOfAccount === AppConstant.MENU_TYPE.NHAPHANGHOA) {
      return {
  ...
  async doSendBillCashiers(billTab: any) {
    ...
    if (this.selectedBillTab.data && this.selectedBillTab.data.isRealId) {
      this.onCompleteBill();
      return;
    }
  ...
  getBillReq(isPayment: boolean) {
    return {
      ...
      amountReceivedByCus: this.selectedBillTab.data.amountReceivedByCus || 0,
  ...
  onChangeDocument = (event: any) => {
    ...
    this.isShowPhieuNhap = this.document.code === 'PC';
    this.getLedgers();
  };
  ...
  editBill = (ledgeHistory: any) => {
    ...
    this.ledgerService
      .getLedgerV2(ledgeId, BillTypeNumber[this.billTypeSelected])
      .subscribe((ledgeInfo) => {
        this.customerBillId =
          this.ledgersWarehouse.find(
            (item) =>
              item.orginalVoucherNumber === ledgeInfo.orginalVoucherNumber,
          )?.customerId || null;
        this.customerBillName = this.goodsTable.getCustomerName(
          this.customerBillId,
        );
        this.ledgerService
          .getLedgersDetailV3(ledgeId)
          .subscribe((ledgeProduct) => {
  ...
  buildBillTab = (ledgeInfo: any, ledgeProduct: any) => {
    ...
    let newBill = {
      ...
      data: {
        ...
        debitCode: ledgeInfo.debit ? ledgeInfo.debit?.code : '',
        ...
        invoiceAdditionalDeclarationCode:
          ledgeProduct[0].invoiceAdditionalDeclarationCode,
        bookDate: moment(ledgeInfo.bookDate).format(
          AppConstant.FORMAT_DATE.VN_DATE_PIPE_SHORT_DATE,
        ),
        orginalBookDate: moment(ledgeInfo.orginalBookDate).format(
          AppConstant.FORMAT_DATE.VN_DATE_PIPE_SHORT_DATE,
        ),
        referenceBookDate: moment(ledgeInfo.referenceBookDate).format(
          AppConstant.FORMAT_DATE.VN_DATE_PIPE_SHORT_DATE,
        ),
        invoiceDate: ledgeInfo.invoiceDate
          ? moment(ledgeInfo.invoiceDate).format(
              AppConstant.FORMAT_DATE.VN_DATE_PIPE_SHORT_DATE,
            )
          : null,
      },
    ...
    this.document = this.documentList.find(
      (item) => item.code === ledgeInfo.type,
    );
    this.filterMonth = ledgeInfo.month;
    this.oldNextStt = ledgeInfo.orginalVoucherNumber
      ? parseInt(ledgeInfo.orginalVoucherNumber.split('-')[2])
      : 0;

    this.selectedBillTab.data.products = [];
    ledgeProduct.forEach((product) => {
      if (
        !product.orginalDescription ||
        !product.orginalDescription.includes('GTGT')
      ) {
    ...
    newBill.data.products = Object.assign(
      [],
      this.selectedBillTab.data.products,
    );
    if (this.billTabs.length > 1) {
    ...
    const productVat = ledgeProduct.find(
      (product) => product.orginalDescription?.includes('GTGT'),
    );
    if (productVat) {
      newBill.data.vat = this.taxRates.find(
        (tax) => tax.code === productVat.invoiceCode,
      ).percent;
      newBill.data.vatAmount = productVat.amount;
      setTimeout(() => {
        this.appBillTable.vatAmountInput = newBill.data.vatAmount;
        this.appBillTable.isTransferStock = true;
        this.appBillTable.billTaxCode = this.taxRates.find(
          (tax) => tax.code === productVat.invoiceCode,
        ).code;
        this.appBillTable.taxRatesComponentBill.value = this.taxRates.find(
          (tax) => tax.code === productVat.invoiceCode,
        );
      }, 0);
