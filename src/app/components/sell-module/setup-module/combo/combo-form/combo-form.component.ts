...
export class ComboFormComponent implements OnInit {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (this.isEdit && this.formData) {
      this.listCombo = this.formData;
    }
  }

  ngOnInit() {}

  dataInit: comboFormModal = {
    id: 0,
    account: '',
    accountName: '',
    detail1: '',
  ...
  addAccount() {
    ...
    this.listCombo.push(Object.assign({}, this.dataInit));
    this.listDetail1 = [];
    this.listDetail2 = [];
  ...
  deleteRow(i, id) {
    this.comboService.deteteRow(id).subscribe((res) => {
  ...
  getTotalCost() {
    let sum = 0;
    for (let item of this.listCombo) {
      sum += item.amount;
    }
    return sum;
  }
  ...
  onSubmit() {
    this.comboService.save(this.listCombo).subscribe((res) => {
      if (res?.code === 400) {
