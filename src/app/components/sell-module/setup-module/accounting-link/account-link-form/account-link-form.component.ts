...
export class AccountLinkFormComponent implements OnInit {
  ...
  checkValidValidator(fieldName: string) {
    return ((this.accountLinkForm.controls[fieldName].dirty ||
      ...
      (this.isInvalidForm && this.accountLinkForm.controls[fieldName].invalid)
      ? 'ng-invalid ng-dirty'
      : '';
  }
  ...
  onSubmit() {
    ...
    if (newData.detail1) {
      let item = this.listDetail1.find((item) => item.code == newData.detail1);
      newData.detailName1 = item.name;
    }
    if (newData.detail2) {
      let item = this.listDetail2.find((item) => item.code == newData.detail2);
