...
export class MenuOfGoodsFormComponent implements OnInit {
  ...
  checkValidValidator(fieldName: string) {
    return ((this.categoryForm.controls[fieldName]?.dirty ||
      ...
      (this.isInvalidForm && this.categoryForm.controls[fieldName]?.invalid)
      ? 'ng-invalid ng-dirty'
      : '';
  }
  ...
  getDetail(id) {
    this.categoryService.getDetail(id).subscribe((res: any) => {
      this.categoryForm.setValue({
        ...
        totalAmountBuy: Number(res.totalAmountBuy),
      });
  ...
  onSubmit() {
    ...
    newData.totalAmountBuy = newData.totalAmountBuy
      ? Number(newData.totalAmountBuy)
      : 0;
    if (this.isEdit) {
