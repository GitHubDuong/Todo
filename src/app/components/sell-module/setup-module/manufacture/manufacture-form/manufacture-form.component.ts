...
interface ManufactureFormModal {
  id?: number;
  account?: string;
  accountName?: string;
  detail1?: string;
...
@Component({
  ...
  styleUrls: ['./manufacture-form.component.scss'],
  styles: [
    `
      :host ::ng-deep {
...
export class ManufactureFormComponent implements OnInit {
  ...
  ManuFactureForm: FormGroup = new FormGroup({});
  isInvalidForm = false;
  ...
  constructor(
    ...
    private manufacureService: ManufactureService,
  ) {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (this.isEdit && this.formData) {
      this.listManufacture = this.formData;
    }
  }
  ngOnInit(): void {}
  dataInit: ManufactureFormModal = {
    id: 0,
    account: '',
    accountName: '',
    detail1: '',
  ...
  chooseCreditCode(event) {
    ...
    const id = this.ManuFactureForm.get('creditCode').value;
  ...
  chooseDetail1(event) {
    ...
    const id =
      this.ManuFactureForm.get('creditCode').value +
      ':' +
      this.ManuFactureForm.get('creditDetailCodeFirst').value;
    this.chartOfAccount.getDetail(id).subscribe((res) => {
  ...
  addAccount() {
    ...
    this.listManufacture.push(Object.assign({}, this.dataInit));
    this.listDetail1 = [];
    this.listDetail2 = [];
  ...
  deleteRow(i, id) {
    this.manufacureService.deteteRow(id).subscribe((res) => {
      this.listManufacture.splice(i, 1);
    });
  }
  closeModal() {
    this.dataInit = {
  ...
  getTotalCost() {
    let sum = 0;
    for (let item of this.listManufacture) {
      sum += item.amount;
    }
    return sum;
  }
  ...
  get stateButtonDisable(): boolean {
    return !this.ManuFactureForm.get('creditDetailCodeFirst').value;
  }

  onSubmit() {
    this.manufacureService.save(this.listManufacture).subscribe((res) => {
      if (res?.code === 400) {
