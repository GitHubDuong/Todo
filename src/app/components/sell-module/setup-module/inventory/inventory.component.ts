...
export class InventoryComponent implements OnInit {
      ...
      const inventories =
        res?.data.reduce((arr, curr, index) => {
          arr.push({
            ...curr,
            no: index + 1,
            image1: curr.image1 && appUtil.baseUrlImage(curr.image1),
          });
          return arr;
        }, []) || [];
      this.result = {
    ...
    this.inventoryService.getListDateInventory().subscribe((res) => {
      const inventoryDates =
        [...new Set(res.map((item) => item))]?.map((date) => {
          return {
            label: new DatePipe('en_US').transform(date, 'dd/MM/yyyy HH:mm:ss'),
            value: date,
          };
        }) || [];
      this.inventoryDates = _.orderBy(inventoryDates, ['value'], ['desc']);
  ...
  getInventories(event?: any): void {
    this.param.page =
      Math.floor(Number(event?.first || 0) / Number(event?.rows || 1)) + 1;
    this.param.pageSize = Number(event?.rows || 20);
    this.inventoryService.getPagingInventory(this.param).subscribe((res) => {
      this.result = res;
    });
  }
  ...
  onDelete(id) {
    let message;
    this.translateService.get('question.delete_inventory').subscribe((res) => {
      message = res;
    });
    this.confirmationService.confirm({
      message: message,
      accept: () => {
        this.inventoryService.deleteInventory(id).subscribe((response: any) => {
          this.getInventories();
        });
      },
    });
  ...
  openDownloadFile(_fileName: string, _ft: string) {
    try {
      var _l = this.inventoryService.getFolderPathDownload(_fileName, _ft);
