...
import { GoodWarehouseExportFormComponent } from './good-warehouse-export-form/good-warehouse-export-form.component';
import { GoodWarehousesFormComponent } from '../good-warehouses/good-warehouses-form/good-warehouses-form.component';
import { GoodWarehouseService } from '../../../service/good-warehouse.service';
import { GoodWarehouseExportService } from 'src/app/service/good-warehouse-export.service';

@Component({
  selector: 'app-good-warehouse-export',
  templateUrl: './good-warehouse-export.component.html',
  styleUrls: [],
...
export class GoodWarehouseExportComponent implements OnInit {
  ...
  @ViewChild('goodWarehousesForm')
  goodWarehousesForm: GoodWarehouseExportFormComponent;
  ...
  constructor(
    ...
    private readonly goodWarehouseExportService: GoodWarehouseExportService,
  ...
  ngOnInit() {
    ...
    this.getCategories();
    this.getGoods();
    // this.goodWarehouseService.stateReCallApi$.subscribe(res => {
    //     if (res) {
    //         this.getGoods();
    //     }
    // })
  ...
  getGoods(event?: any, isExport: boolean = false): void {
    ...
    this.pendingRequest = this.goodWarehouseExportService
      .getGoodWarehouse(params)
  ...
  getDetail(id) {
    this.isEdit = true;
    this.goodWarehousesForm.getDetail(id);
    this.display = true;
  }
  ...
  import(evt) {
    ...
    reader.onload = (e: any) => {
      ...
      // this.goodsServices.importExcelListOfGoods(dataImport).subscribe(res => {
  ...
  getName(code, list: any[]): string {
    const detail = list?.find((x) => x.code === code);
    if (!detail) return '';
