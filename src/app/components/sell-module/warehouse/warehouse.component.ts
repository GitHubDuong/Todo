...
export class WarehouseComponent implements OnInit {
        ...
        (res) => {
          ...
          if (status == 400) {
            this.confirmationService.confirm({
              ...
              accept: () => {
                this.goodWarehouseService
                  .completeBill(true, this.currentBill.products)
                  .subscribe(
                    (res) => {
                      this.messageService.add({
                        severity: 'success',
  ...
  async viewDetail(msg: NotificationResult) {
    ...
    this.billService.getBillDetail(msg.billId).subscribe((res: any) => {
      this.currentBill = {
        id: msg.billId,
        deskName: this.desks.find((x) => x.id === res.data.deskId)?.name,
        floorName: this.floors.find((x) => x.id === res.data.floorId)?.name,
        payPrice: 0,
        products: [],
        tabId: '',
  ...
  orderPageNavigate():void {
    this.websiteOrdersService
      .billToOrderProduce(this.currentBill.id)
      .subscribe((res: any) => {
        AppUtil.scrollToTop();
        this.messageService.add({
          severity: 'success',
        });
        this.router.navigate(['uikit/production-orders-new'], { queryParams: { orderNumber: res.procedureNumber } });
      })
  }
}
