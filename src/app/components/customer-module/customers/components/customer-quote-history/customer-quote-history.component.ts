...
export class CustomerQuoteHistoryComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.param.customerId = Number(this.route.snapshot.paramMap.get('id') || 0);
      ...
      (res) => {
        ...
        this.result.data =
          this.result?.data?.reduce((arr, curr, index) => {
            arr.push({
              ...curr,
              no: (this.param.page - 1) * this.param.pageSize + index + 1,
            });
            return arr;
          }, []) || [];
        this.customer = {
  ...
  getCustomerQuoteDetail(customerQuoteId): void {
    ...
    this.customerService.getListCustomerQuoteDetail(param).subscribe((res) => {
      this.resultDetail =
  ...
  onPreview(item): void {
    this.billPdfGeneratorService.prepareAndGenerateBaoGia(
      item.customerId,
      item.id,
    );
  }
