...
export class CustomersFormComponent extends BaseAccountComponent implements OnInit, OnChanges {
  ...
  @Input('showAccountingConnection') showAccountingConnection: boolean = false;
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (
      ...
      this.customerForm.setValue({
        ...
        identityCardAddressInCard: this.formData.identityCardAddressInCard,
  ...
  private buildFormAccount(data: any) {
    ...
    this.getDetail(data.id, (res) => {
      if (res.data.debit) {
        res.data.debit.displaySelected =
          res.data.debit.code + ' - ' + res.data.debit.name;
      }
      if (res.data.debitDetailFirst) {
        res.data.debitDetailFirst.displaySelected =
          res.data.debitDetailFirst.code + ' - ' + res.data.debitDetailFirst.name;
      }
      if (res.data.debitDetailSecond) {
        res.data.debitDetailSecond.displaySelected =
          res.data.debitDetailSecond.code + ' - ' + res.data.debitDetailSecond.name;
      }
  ...
  onSubmit() {
    ...
    customerApiAction.subscribe((cusResponse: any) => {
      ...
      if (this.isEdit) {
        ...
        .updateCustomerTax(taxRequest, this.formData.id)
        .subscribe((_) => {
          this.onCancel.emit({});
        });
      } else {
  ...
  cleanObject(data) {
    ...
    return {
      ...
      debitDetailCodeFirst: this.fc['debitDetailCodeFirst'].value?.code || '',
      debitDetailCodeSecond:
  ...
  doAttachFile(event: any): void {
    ...
    this.userService.uploadFiles(image).subscribe((res) => {
      if (res && res.body) {
        this.customerForm.controls['avatar'].setValue(res.body.imageUrl);
      }
    });
  }
