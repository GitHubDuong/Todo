...
@Component({
  ...
  styleUrls: ['../../../../assets/demo/badges.scss', './customers.component.scss'],
})
export class CustomersComponent extends AppComponentBase implements OnInit {
  ...
  @ViewChild('contractForm') contractFormComponent: ContactHistoryFormComponent | undefined;
  ...
  cols: any[] = [
    ...
    {
      header: this.router.url.includes('suppliers') ? 'label.supplier_name' : 'label.customer_name',
      value: 'fullName',
      width: 'w-2',
      display: true,
      classify: 'personal_info',
  ...
  ngOnInit() {
    ...
    this.showAccountingConnection = this.authService.user?.roleName.some((role) =>
      [AppConstant.USER_TYPE.SUPER_ADMIN, AppConstant.USER_TYPE.ADMIN_BRANCH].includes(role),
    );

    this.route.queryParams.subscribe((params) => {
      this.displayHistories = false;
      const customerId = params['customer-id'];
      if (!customerId) {
        return;
      }
      // Do not reload the first time open component
  ...
  getStatisData() {
    let customerType = null,
      customerTypeName = 'Khách';
    const url = this.router.url;
    switch (true) {
      case url.includes('/uikit/customers'):
        customerType = CustomerType.Customer;
        break;
      case url.includes('/uikit/suppliers'):
        customerType = CustomerType.Supplier;
        customerTypeName = 'NCC';
        break;
      case url.includes('/uikit/web-customers'):
    ...
    this.statisChartCustomerData = new IsFunnelChartModel({
      labels: [
        `${customerTypeName} truy cập`,
        `${customerTypeName} tiềm năng`,
        `${customerTypeName} chất lượng`,
        `${customerTypeName} mua hàng`,
      ],
      backgroundColors: ['#FF6384', '#FFCE56', '#36A2EB', '#66BB6A'],
      data: ['2.000', '1.500', '1.000', '500'],
    });

    this.customerService.getChartBirthDay(customerType).subscribe((response: any) => {
      ...
      this.statisticsBirthdayInMonthData = {
        labels: userStatisticsData.birthDayOfUsers.map((a) =>
          AppUtil.translateWithParams(this.translateService, 'label.number_month', { month: a.month }),
        ),
        datasets: [
          {
  ...
  getJobsAndStatus() {
    this.customerJobService.getJobAndStatusExistingInCustomerHistories(0).subscribe((res) => {
      AppUtil.scrollToTop();
  ...
  getCategory() {
    ...
    this.categoryService.getPaging(params).subscribe((res) => {
  ...
  getCustomers(event?: any, isExport: boolean = false): void {
    ...
    if (isExport) {
      ...
      this.getParams['FromDate'] = this.dateAddCustomerFrom ? moment(this.dateAddCustomerFrom).format('YYYY-MM-DD') : null;
      this.getParams['ToDate'] = this.dateAddCustomerTo ? moment(this.dateAddCustomerTo).format('YYYY-MM-DD') : null;
      ...
      this.getParams['customerClassficationId'] = this.customerClassficationId;
      this.getParams['area'] = this.area;

      Object.keys(this.getParams).forEach((k) => this.getParams[k] == null && delete this.getParams[k]);
      this.customerService.getExcelReport(this.getParams).subscribe((res: any) => {
        AppUtil.scrollToTop();
      ...
      this.pendingRequest = this.customerService.getPagingCustomer(this.getParams).subscribe((response: TypeData<Customer>) => {
        AppUtil.scrollToTop();
        this.getStatisData();
  ...
  getDetail(customerId: number) {
    forkJoin([
      ...
      this.customerTaxService.getCustomerTaxDetailByCustomerId(customerId),
  ...
  async getContractForm(customer: Customer) {
    this.customerService.getCustomerDetail(customer.id).subscribe((response: any) => {
      this.selectedCustomer = response.data;
  ...
  onDelete(userId) {
    let message;
    this.translateService.get('question.delete_customer_content').subscribe((res) => {
      message = res;
    });
    this.confirmationService.confirm({
      message: message,
      accept: () => {
        this.customerService.deleteCustomer(userId).subscribe((response: Customer) => {
          this.messageService.add({
            severity: 'success',
  ...
  getListAccountCustomer() {
    this.customerClassificationService.getAllCustomerClassification().subscribe((response: any) => {
      this.customerTypes = response.data;
    });
  }

  getListDistrict() {
    this.districtService.getListDistrict().subscribe((response: District[]) => {
      this.districts = response;
    });
  }

  getListProvince() {
    this.provinceService.getListProvince().subscribe((response: Province[]) => {
  ...
  getListContractType() {
    this.contractTypeService.getAllContractType(1).subscribe((response: any) => {
  ...
  import(evt) {
    ...
    reader.onload = (e: any) => {
      ...
      if (datas && datas?.length > 0) {
        datas.forEach((element) => {
          ...
          objProps.forEach((item, index) => {
            ...
            } else if (item == 'birthday' && element[index] !== undefined && element[index] !== null) {
              objItem[item] = AppUtil.adjustDateOffset(AppUtil.convertStringToDate(element[index]));
            } else {
      ...
      this.customerService.importExcel(dataImport, this.getParams.type).subscribe(
        (res) => {
          this.messageService.add({
            severity: 'success',
  ...
  onAddJob(customerId) {
    ...
    var queryParams = { status: 'addJobs' };
    this.router.navigate(['/uikit/workflow'], { queryParams });
  }

  checkTotalMount(totalAmountPay: any) {
    if (totalAmountPay == null) {
      return null;
    }
    // Lọc các đối tượng lớn nhất và nhỏ hơn totalAmountPay
    const filteredArray = this.categories.filter((item) => item.totalAmountBuy !== null && item.totalAmountBuy < totalAmountPay);

    // Nếu không có đối tượng thỏa mãn điều kiện, trả về null hoặc giá trị mặc định khác
    if (filteredArray.length === 0) {
      return null; // hoặc return defaultValue;
    }

    // Sắp xếp mảng theo giảm dần
    filteredArray.sort((a, b) => b.totalAmountBuy - a.totalAmountBuy);

    // Trả về giá trị lớn nhất
    this.listCategory = filteredArray;
    return filteredArray;
  }

  showDialogUpdateTotalMount(customer) {
  ...
  onSaveChange() {
    this.customerService
      .getCustomerDetail(this.customerSelected.id)
      .pipe(
        switchMap((res) => {
          res.data.priceList = this.listCategory.length > 0 ? this.listCategory[0].code : 0;
          return this.customerService.updateCustomer(res.data, this.customerSelected.id);
        }),
      )
      .subscribe(() => {
  ...
  onSync() {
    this.customerService.syncCustomer(this.router.url.includes('/uikit/suppliers') ? 1 : 0).subscribe(() => {
      this.messageService.add({
        severity: 'success',
