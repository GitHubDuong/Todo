...
@Component({
  ...
  styleUrls: ['../../../../assets/demo/badges.scss', 'salary-advance.component.scss'],
})
export class SalaryAdvanceComponent implements OnInit {
  appConstant = AppConstants;
  @ViewChild('decideForm') SalaryAdvanceFormComponent: SalaryAdvanceFormComponent | undefined;
  ...
  ngOnInit(): void {
    ...
    forkJoin(listBranch, listDepartment).subscribe(([branch, department]) => {
      this.loading = false;
  ...
  onFilterBranch(branchId: any) {
    this.selectedListDepartment = filter(this.listDepartment, ['branchId', branchId]);
  ...
  getSalaryAdvanceForm(item) {
    this.salaryAdvanceService.getSalaryAdvanceDetail(item.id).subscribe((response: any) => {
  ...
  onDelete(item) {
    this.confirmationService.confirm({
      header: AppUtil.translate(this.translateService, 'question.delete_confirm'),
      message: AppUtil.translate(this.translateService, 'question.delete_confirm'),
      accept: () => {
        this.salaryAdvanceService.deleteSalaryAdvance(item.id).subscribe(
          (res) => {
  ...
  UpdateSalaryToAccountant(isInternal: number): void {
    this.loading = true;
    this.pendingRequest = this.salaryAdvanceService.AddLedger(this.getParams.month, isInternal).subscribe((data) => {
      this.loading = false;
    });
  }
}
