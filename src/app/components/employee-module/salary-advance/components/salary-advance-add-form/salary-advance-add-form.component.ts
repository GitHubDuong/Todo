...
export class SalaryAdvanceAddFormComponent implements OnInit {
  ...
  initDefaultValue() {
    if (this.isEdit) {
      ...
      this.salaryAdvanceItem.branchId = cloneDeep(this.selectedItem.branchId);
      this.totalRecords = this.selectedItem.items.length || 0;
    } else {
      this.listSalaryAdvanceDetail.branchId = cloneDeep(this.listBranch[0].id);
  ...
  checkValidationDetail() {
    ...
    if (
      !moment(date, AppConstant.FORMAT_DATE.VN_DATE_PIPE_SHORT_DATE).isValid()
    ) {
  ...
  onDelete(index: number) {
    if (this.listSalaryAdvanceItemOriginal.length == 1) {
      this.listSalaryAdvanceItemOriginal = [{ ...this.salaryAdvanceItem }];
    } else {
