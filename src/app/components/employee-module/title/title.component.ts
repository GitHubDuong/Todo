...
import { TITLE_COLUMNS } from '@components/employee-module/title/title.config';
...
@Component({
  ...
  styleUrls: ['../../../../assets/demo/badges.scss', 'title.component.scss'],
})
export class TitleComponent implements OnInit {
  public appConstant = AppConstant;
  @ViewChild('PositionForm') TitleFormComponent: TitleFormComponent | undefined;
  ...
  onSearch(event) {
    if (event.key === 'Enter') {
      this.getPositions(this.defaultLoad);
    }
  }
  ...
  getPositions(event?: any, isExport: boolean = false): void {
    ...
    this.pendingRequest = this.PositionService.getListPosition(this.getParams).subscribe((response: TypeData<Position>) => {
      AppUtil.scrollToTop();
  ...
  getDetail(PositionId) {
    this.PositionService.getPositionDetail(PositionId).subscribe((response: Position) => {
  ...
  onDelete(PositionId) {
    let message;
    this.translateService.get('question.delete_Position_content').subscribe((res) => {
      message = res;
    });
    this.confirmationService.confirm({
      message: message,
      accept: () => {
        this.PositionService.deletePosition(PositionId).subscribe((response: any) => {
          this.getPositions();
        });
      },
    });
