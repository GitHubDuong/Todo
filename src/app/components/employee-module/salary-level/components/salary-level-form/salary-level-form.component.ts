...
export class SalaryLevelFormComponent implements OnInit {
  ...
  checkValidValidator(fieldName: string) {
    return (this.salaryLevelForm.controls[fieldName] &&
      (this.salaryLevelForm.controls[fieldName].dirty ||
        this.salaryLevelForm.controls[fieldName].touched) &&
      this.salaryLevelForm.controls[fieldName].invalid) ||
      (this.isInvalidForm && this.salaryLevelForm.controls[fieldName].invalid)
      ? 'ng-invalid ng-dirty'
      : '';
  }
  onSubmit() {
  ...
  cleanObject(data) {
    ...
    newData.positionId = newData.positionId || 0;
    newData.positionName = this.lstPositions.find(
      (item) => item.id === newData.positionId,
    ).name;
