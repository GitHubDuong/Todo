...
export class StoreFormComponent implements OnInit, OnChanges {
  ...
  onReset() {
    ...
    // Default branch when creating
    const defaultBranch = this.listBranch.find(x => x.code == "HS");
    if(defaultBranch) {
  ...
  checkValidMultiValidator(fieldNames: string[]) {
    for (let i = 0; i < fieldNames.length; i++) {
      if (
        ...
        (this.isInvalidForm && this.storeForm.controls[fieldNames[i]].invalid)
      ) {
        return true;
      }
    }
  ...
  onSubmit() {
    ...
    let newData = this.cleanObject(AppUtil.cleanObject(this.storeForm.value));
    if (this.isEdit) {
      this.storeService
