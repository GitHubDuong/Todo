...
export class StoreComponent implements OnInit {
  ...
  getStores(event?: any, isExport: boolean = false): void {
    ...
    if (isExport) {
      this.storeService.getExcelReport(this.getParams).subscribe((res: any) => {
        AppUtil.scrollToTop();
  ...
  getDetail(storeId) {
    this.storeService.getStoreDetail(storeId).subscribe((response: Store) => {
  ...
  onDelete(storeId) {
    ...
    this.confirmationService.confirm({
      ...
      message: message,
      accept: () => {
        this.storeService.deleteStore(storeId).subscribe((response: any) => {
          this.getStores();
        });
      },
    });
