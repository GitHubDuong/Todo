...
export class ShiftComponent implements OnInit {
  ...
  @ViewChild('SymbolForm') SymbolFormComponent: ShiftFormComponent | undefined;
  ...
  getSymbols(event?: any, isExport: boolean = false): void {
    ...
    this.pendingRequest = this.SymbolService.getListSymbol(this.getParams).subscribe((response: TypeData<Symbol>) => {
      AppUtil.scrollToTop();
  ...
  getDetail(SymbolId) {
    this.SymbolService.getSymbolDetail(SymbolId).subscribe((response: Symbol) => {
  ...
  onDelete(SymbolId) {
    let message;
    this.translateService.get('Bạn có chắn chắn muốn xóa ca này không').subscribe((res) => {
      message = res;
    });
    this.confirmationService.confirm({
      message: message,
      accept: () => {
        this.SymbolService.deleteSymbol(SymbolId).subscribe((response: any) => {
          this.getSymbols();
        });
      },
    });
