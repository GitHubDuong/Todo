...
@Component({
  ...
  styleUrls: ['../../../../assets/demo/badges.scss', './user.component.scss'],
})
export class UserComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.activatedRoute.queryParamMap.subscribe((param) => {
      if (param.get('age')) {
        ...
        this, (this.getParams.endAge = (parseInt(param.get('age')) + 4).toString());
      }
    ...
    this.$textSearch.pipe(debounceTime(500)).subscribe((text) => {
      this.getUsers();
    });
  }
  ...
  getUsers(event?: any, isExport: boolean = false): void {
    ...
    if (this.chooseSearhBirth) {
      if (params.startDate) {
        params.startDate = moment(params.startDate).format('YYYY-MM-DDTh:mm:ss');
      }
      if (params.endDate) {
        params.endDate = moment(params.endDate).format('YYYY-MM-DDTh:mm:ss');
      }
    } else {
    ...
    Object.keys(params).forEach((k) => {
      if (params[k] == null || (k.includes('Id') && params[k] === 0) || params[k] === 'Invalid date') {
        delete params[k];
      }
    });
    ...
    this.pendingRequest = this.userService.getPagingUser(params).subscribe((response: TypeData<User>) => {
      AppUtil.scrollToTop();
  ...
  getTotalRequestPassword() {
    this.userService.getTotalRequestPassword().subscribe((response: any) => {
  ...
  onDelete(userId) {
    let message;
    this.translateService.get('question.delete_user_content').subscribe((res) => {
      message = res;
    });
    this.confirmationService.confirm({
      message: message,
      accept: () => {
        this.userService.deleteUser(userId).subscribe((response: User) => {
          this.first = 0;
          this.getUsers();
        });
      },
  ...
  getListContractType() {
    this.contractTypeService.getAllContractType(0).subscribe((response: any) => {
  ...
  getUserStatistics() {
    this.userService.userStatistics().subscribe((response) => {
      ...
      this.userStatisticsGenderData = {
        ...
        datasets: [
          {
            data: [userStatisticsData.totalMale, userStatisticsData.totalFemale],
  ...
  resetPassword() {
    const checkedUser = (this.lstUsers || []).filter((user: any) => user.checked).map((user: any) => user.id);
    this.confirmationService.confirm({
      key: 'resetPassword',
      message: 'Bạn có chắc chắn muốn reset password?',
      accept: () => {
        this.userService.resetPassword(checkedUser).subscribe((response: any) => {
          this.first = 0;
          this.getUsers();
        });
      },
      ...
      const request =
        arraylist?.reduce((arr: any[], curr: any, index) => {
          arr.push({
            ...
            sendSalaryDate: AppUtil.adjustDateOffset(AppUtil.convertStringToDate(curr.sendSalaryDate)),
            identifyCreatedDate: AppUtil.adjustDateOffset(AppUtil.convertStringToDate(curr.identifyCreatedDate)),
          });
          return arr;
        }, []) || [];
  ...
  printContract(contractType) {
    var ref = this.dialogService.open(PrintContractComponent, {
      ...
      baseZIndex: 10000,
      header: contractType.name ? 'In ' + contractType.name.toLowerCase() : 'In hợp đồng lao động',
    });
  }
