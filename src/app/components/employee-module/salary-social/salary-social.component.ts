    ...
    this.salarySocialService.getSalarySocial().subscribe((res) => {
      ...
      this.salarySocial.map((item) => {
        if (item.valueCompany) {
          const [preNumberInString, postNumberInString] = item.valueCompany
            .toString()
            .split('.');
          item.valueCompany = `${preNumberInString ? preNumberInString : ''}${
            postNumberInString ? ', ' + postNumberInString : ''
          }`;
        }
        if (item.valueUser) {
          const [preNumberInString, postNumberInString] = item.valueUser
            .toString()
            .split('.');
          item.valueUser = `${preNumberInString ? preNumberInString : ''}${
            postNumberInString ? ', ' + postNumberInString : ''
          }`;
        }
      });
