...
export class ImportProcessComponent implements OnInit, AfterContentInit {
  showForm = false;
  selectedItem: any;
  loading = false;
  data: any[] = [];
  ...
  onLoad(event: any = undefined) {
    ...
    this.importProcessService.getByPage(params).subscribe(
      (res: any) => {
        this.loading = false;
        this.data = res.data;
  ...
  onDelete(id) {
    this.importProcessService.delete(id).subscribe(
      (res: any) => {
  ...
  onAccept(item: any) {
    this.importProcessService.accept(item.id).subscribe((item: any) => {
  ...
  onPrint(item: any) {
    this.importProcessService.export(item.id).subscribe((res: any) => {
  ...
  onAttackFiles(row: any, event: any) {
    this.confirmService.confirm({
      ...
      accept: () => {
        this.importProcessService.uploadDocument(row.id, event.target?.files[0]).subscribe((res: any) => {
  ...
  onNotAccept(id) {
    this.importProcessService.notAccept(id).subscribe((res: any) => {
