...
import { LedgerReceiptService } from '@components/procedure-module/ledger-receipt/ledger-receipt.service';
import { OrderStatus } from '@utilities/app-enum';
import { ConfirmationService, MenuItem } from 'primeng/api';

@Component({
  selector: 'app-ledger-receipt',
  templateUrl: './ledger-receipt.component.html',
  styleUrls: ['./ledger-receipt.component.scss'],
})
export class LedgerReceiptComponent implements OnInit, AfterContentInit {
  showForm = false;
  selectedItem: any;
  loading = false;
  data: any[] = [];
  ...
  onLoad(event: any = undefined) {
    ...
    this.ledgerReceiptService.getByPage(params).subscribe(
      (res: any) => {
        this.loading = false;
        this.data = res.data;
  ...
  onDelete(id) {
    this.ledgerReceiptService.delete(id).subscribe(
      (res: any) => {
  ...
  onAccept(item: any) {
    this.ledgerReceiptService.accept(item.id).subscribe((item: any) => {
  ...
  onNotAccept(id) {
    this.ledgerReceiptService.notAccept(id).subscribe((res: any) => {
  ...
  onPrint(item: any) {
    this.ledgerReceiptService.export(item.id).subscribe((res: any) => {
  ...
  onAttackFiles(row: any, event: any) {
    this.confirmService.confirm({
      ...
      accept: () => {
        this.ledgerReceiptService.uploadDocument(row.id, event.target?.files[0]).subscribe((res: any) => {
