...
import { LedgerPaymentService } from '@components/procedure-module/ledger-payment/ledger-payment.service';
import { OrderStatus } from '@utilities/app-enum';
import { ConfirmationService, MenuItem } from 'primeng/api';

@Component({
  selector: 'app-ledger-payment',
  templateUrl: './ledger-payment.component.html',
  styleUrls: ['./ledger-payment.component.scss'],
})
export class LedgerPaymentComponent implements OnInit, AfterContentInit {
  showForm = false;
  selectedItem: any;
  loading = false;
  data: any[] = [];
  ...
  onLoad(event: any = undefined) {
    ...
    this.ledgerPaymentService.getByPage(params).subscribe(
      (res: any) => {
        this.loading = false;
        this.data = res.data;
  ...
  onDelete(id) {
    this.ledgerPaymentService.delete(id).subscribe(
      (res: any) => {
  ...
  onAccept(item: any) {
    this.ledgerPaymentService.accept(item.id).subscribe((item: any) => {
  ...
  onNotAccept(id) {
    this.ledgerPaymentService.notAccept(id).subscribe((res: any) => {
  ...
  onPrint(item: any) {
    this.ledgerPaymentService.export(item.id).subscribe((res: any) => {
  ...
  onAttackFiles(row: any, event: any) {
    this.confirmService.confirm({
      ...
      accept: () => {
        this.ledgerPaymentService.uploadDocument(row.id, event.target?.files[0]).subscribe((res: any) => {
