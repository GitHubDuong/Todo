...
@Component({
  ...
  styleUrls: ['../../../../assets/demo/badges.scss', './timekeeping-report.component.scss'],
})
export class TimekeepingReportComponent implements OnInit {
  ...
  getTimekeepingHistory(event?: any, isExport: boolean = false): void {
    ...
    this.pendingRequest = this.timeKeepingReport
      .getTimekeepingReportV2(this.getParams)
  ...
  displaySymbolTimeKeeping(date, index: number) {
    ...
    //
    // const userHistory = this.lstTimekeepingHistory[index];
    // userHistory.histories.forEach((item) => {
    ...
    // });
    //
    // let overtimeHours = userHistory.overtimesHistories.find(x =>
    //   moment(x.date).format(this.appConstant.FORMAT_DATE.NORMAL_DATE,) == date
    // )?.totalHours || 0;
    //
    // if(symbols.length == 0) {
    //   symbols += "0";
    // }
    //
    // symbols = symbols.trim().replace(/,$/g, ' ');
    // if(overtimeHours > 0) {
    //   symbols += ', ' +  overtimeHours;
    // }
    return symbols;
  }
