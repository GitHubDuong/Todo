...
import { LanguagePage, TypeData } from '../../../models/common.model';
...
export class IntroWebComponent implements OnInit {
      ...
      (res) => {
        ...
        this.result = {
          ...
          data: res?.data?.map((item) => {
            return {
              ...item,
              typeName: AppUtil.getLanguageTypes().find(
                (x) => x.value === item.type,
              ).name,
              introduceTypeName: this.getIntroduceTypeName(item.introduceTypeId),
            };
          }),
        };
      },
  ...
  getIntroduceDetail(item) {
    ...
    this.introduceService.getIntroduceDetail(item.id).subscribe(
      (res) => {
        this.formData = res;
      },
      (error) => {
