...
export class MenuWebEditComponent implements OnInit {
  ...
  @Input() set formData(value) {
    if (value?.id) {
      ...
      if (this.menuModel.icon) {
        ...
        this.iconFile = this.menuModel.icon;
      }
  ...
  onRemoveImages() {
    this.fileLinks = this.fileLinks.filter(
      (x) => !this.selectedImages.includes(x.fileId),
    );
  }
  ...
  onSave() {
    ...
    this.menuModel.numberItem = Number(this.menuModel.numberItem) ?? 0;
    ...
    action.subscribe(
      (res) => {
        const msgKey = this.menuModel?.id ? 'success.update' : 'success.create';
