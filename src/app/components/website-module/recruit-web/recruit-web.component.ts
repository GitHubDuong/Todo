...
export class RecruitWebComponent implements OnInit {
            ...
            return {
              ...
              groupName:
                item.group === CareerGroupType.Office
                  ...
                  : '',
              workingMethodName:
                item.workingMethod === WorkingMethodType.FullTime
                  ? 'Toàn thời gian'
                  : item.workingMethod === WorkingMethodType.PartTime
                  ? 'Bán thời gian'
                  : item.workingMethod === WorkingMethodType.Shift
                  ? 'Ca'
                  : '',
            };
          }),
  ...
  getCareerDetail(item) {
    this.display = true;
    this.careerService.getCareerDetail(item.id).subscribe(
      (res) => {
    this.formData = res;
      },
      (error) => {
