...
export class CompanyFormComponent implements OnInit {
  ...
  getLastInfo() {
    this.companyService.getLastCompanyInfo().subscribe((response: any) => {
      ...
      this.companyForm.setValue({
        ...
        accordingAccountingRegime: this.formData.accordingAccountingRegime,
  ...
  checkValidMultiValidator(fieldNames: string[]) {
    for (let i = 0; i < fieldNames.length; i++) {
      if (
        ...
        (this.isInvalidForm && this.companyForm.controls[fieldNames[i]].invalid)
      ) {
        return true;
      }
    }
  ...
  onSubmit() {
    ...
    let newData = this.cleanObject(AppUtil.cleanObject(this.companyForm.value));
  ...
  doAttachFileLogo(event: any): void {
    ...
    this.companyService.uploadFiles(image).subscribe((res) => {
      if (res.body) {
        this.companyForm.controls['fileLogo'].setValue(res.body.imageUrl);
      }
    });
  }
